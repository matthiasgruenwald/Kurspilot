...
function buildWixSource(payloadDir) {
  ...
  return [
    ...
    '      <Publish Dialog="ExitDialog" Control="Finish" Event="DoAction" Value="LaunchKurspilotAfterInstall" Condition="WIXUI_EXITDIALOGOPTIONALCHECKBOX = 1 AND NOT Installed" Order="1" />',
    '    </UI>',
    ...
    '    </Feature>',
    '    <CustomAction Id="LaunchKurspilotAfterInstall" Directory="INSTALLFOLDER" ExeCommand="[INSTALLFOLDER]bin\\kurspilot-setup.vbs --after-install" Execute="immediate" Return="asyncNoWait" Impersonate="yes" />',
    '  </Package>',
    '</Wix>',
    '',
  ].join("\n");
