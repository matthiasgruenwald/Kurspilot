...
if (!MOODLE_URL || !MOODLE_TOKEN) {
  process.stderr.write(
    ...
    "  node moodle-mcp-core.js https://moodle.example.de/moodle DEIN_TOKEN\n"
  );
  process.exit(1);
}
...
function toolsForProfile() {
  if (!isReadOnlyProfile()) return CORE_TOOLS;
  return CORE_TOOLS.filter(tool => CORE_READ_ONLY_TOOL_NAMES.has(tool.name));
}
...
async function executeTool(name, args) {
  if (isReadOnlyProfile() && !CORE_READ_ONLY_TOOL_NAMES.has(name)) {
  ...
  if (method === "initialize") {
    send({
      ...
      result: {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: "moodle-mcp-core", version: "1.0.0" },
      },
    });
    return;
  }
