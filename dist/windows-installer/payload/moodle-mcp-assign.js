...
if (!MOODLE_URL || !MOODLE_TOKEN) {
  process.stderr.write(
    ...
    "  node moodle-mcp-assign.js https://moodle.example.de/moodle DEIN_TOKEN\n"
  );
  process.exit(1);
}
...
function toolsForProfile() {
  if (!isReadOnlyProfile()) return ASSIGN_TOOLS;
  return ASSIGN_TOOLS.filter(tool => ASSIGN_READ_ONLY_TOOL_NAMES.has(tool.name));
}
...
async function executeTool(name, args) {
  if (isReadOnlyProfile() && !ASSIGN_READ_ONLY_TOOL_NAMES.has(name)) {
  ...
  if (method === "initialize") {
    send({
      ...
      result: {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: "moodle-mcp-assign", version: "1.0.0" },
      },
    });
    return;
  }
