...
if (!MOODLE_URL || !MOODLE_TOKEN) {
  process.stderr.write(
    ...
    "  node moodle-mcp-label.js https://moodle.example.de/moodle DEIN_TOKEN\n"
  );
  process.exit(1);
}
...
function toolsForProfile() {
  if (!isReadOnlyProfile()) return LABEL_TOOLS;
  return LABEL_TOOLS.filter(tool => LABEL_READ_ONLY_TOOL_NAMES.has(tool.name));
}
...
async function executeTool(name, args) {
  if (isReadOnlyProfile() && !LABEL_READ_ONLY_TOOL_NAMES.has(name)) {
  ...
  if (method === "initialize") {
    send({
      ...
      result: {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: "moodle-mcp-label", version: "1.0.0" },
      },
    });
    return;
  }
