...
if (!MOODLE_URL || !MOODLE_TOKEN) {
  process.stderr.write(
    ...
    "  node moodle-mcp-url.js https://moodle.example.de/moodle DEIN_TOKEN\n"
  );
  process.exit(1);
}
...
function toolsForProfile() {
  if (!isReadOnlyProfile()) return URL_TOOLS;
  return URL_TOOLS.filter(tool => URL_READ_ONLY_TOOL_NAMES.has(tool.name));
}
...
async function executeTool(name, args) {
  if (isReadOnlyProfile() && !URL_READ_ONLY_TOOL_NAMES.has(name)) {
  ...
  if (method === "initialize") {
    send({
      ...
      result: {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: "moodle-mcp-url", version: "1.0.0" },
      },
    });
    return;
  }
