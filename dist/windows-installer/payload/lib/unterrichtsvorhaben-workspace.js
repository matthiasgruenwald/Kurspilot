...
const EXISTING_WORKSPACE_DECISION_OPTIONS = [
  'Zusammenfassen',
  'Fortsetzen',
  'Ueberarbeiten',
...
function buildTeacherFacingText({ mode, relativePath, existingFiles }) {
  if (mode === 'existing') {
    ...
    const lines = [
      ...
      'Vorhandene Dateien:',
      fileList,
      'Du kannst den Stand zusammenfassen, fortsetzen, ueberarbeiten oder nach ausdruecklicher Bestaetigung neu anlegen/ueberschreiben.',
    ];
...
function setupUnterrichtsvorhabenWorkspace(baseDirOrFields, fieldsOrOptions, maybeOptions) {
  ...
  const resolvedWorkspacePath = invocation.fields.materialFolder
    ? resolveWorkspacePathFromMaterialFolder(invocation.fields.materialFolder, invocation.options)
    : resolveLocalContextPath(
        getUnterrichtsvorhabenPath(
  ...
  if (existingFiles.length > 0) {
    return {
      ...
      decisionOptions: EXISTING_WORKSPACE_DECISION_OPTIONS,
...
function resolveWorkspacePathFromMaterialFolder(materialFolder, options) {
  const startkontext = resolveKurspilotStartkontextFromWegweiser(materialFolder, {
    readWorkspaceSetting: options.readWorkspaceSetting,
    workspaceConfigOptions: options.workspaceConfigOptions,
  });
  const contextFileName = path.basename(startkontext.absolutePath).toLowerCase();
  const isContextFile = contextFileName === 'context.md';

  return {
    contextRoot: startkontext.contextRoot,
    relativePath: isContextFile ? path.dirname(startkontext.relativePath) : startkontext.relativePath,
    absolutePath: isContextFile ? path.dirname(startkontext.absolutePath) : startkontext.absolutePath,
  };
}
