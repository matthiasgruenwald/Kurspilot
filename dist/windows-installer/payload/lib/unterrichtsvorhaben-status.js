...
function readTableField(markdown, fieldName) {
  if (typeof markdown !== 'string') {
    return null;
  }

  const escapedFieldName = fieldName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const match = markdown.match(new RegExp(`\\|\\s*${escapedFieldName}\\s*\\|\\s*([^|]+?)\\s*\\|`, 'm'));
...
function readListSection(markdown, heading) {
  ...
  return match[1]
    ...
    .filter(Boolean)
    .map((lineMatch) => lineMatch[1].trim())
    .filter((line) => line && line !== '_(noch nicht erfasst)_');
}
...
function renderUnterrichtsvorhabenStatus(fields) {
  ...
  const implementationPoint = markdownOrPlaceholder(fields.implementationPoint);
  const moodleEntries = renderListItems(fields.moodleEntries);
  ...
  return [
    ...
    `| Letzter Umsetzungspunkt | ${implementationPoint} |`,
    '',
    '## Moodle-Ergebnisse',
    moodleEntries,
    '',
...
function readUnterrichtsvorhabenStatusField(markdown, fieldName) {
  return readTableField(markdown, fieldName);
}

function readUnterrichtsvorhabenStatusList(markdown, heading) {
  return readListSection(markdown, heading);
}
