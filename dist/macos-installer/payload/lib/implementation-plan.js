...
const { lookupActivitySupport } = require('./activity-registry');
...
const PRINCIPLES = Object.freeze({
  ...
  QUIZ_LERNCHECK_MODE_DEFAULT:
    'Testaktivität ohne Modusangabe -> Lernstandscheck (mode="lernstandscheck", unbegrenzte Versuche, beste Bewertung).',
...
function normalizeQuestionBankPart(value) {
  if (typeof value !== 'string') return '';
  return value.trim().replace(/\s+/g, ' ');
}

function buildQuestionBankName(plan, options) {
  if (options.name) {
    return normalizeQuestionBankPart(options.name);
  }

  const parts = [];
  const seen = new Set();

  for (const rawPart of [options.courseName, options.projectName, options.topicName, options.contentName]) {
    const part = normalizeQuestionBankPart(rawPart);
    if (!part) continue;
  ...
  return `Kurs ${plan.courseId}`;
}

/**
 * Legt die benannte Kurs-Fragensammlung als Planungsentscheidung fest.
 * Die Lehrkraft kann den vorgeschlagenen Namen bestaetigen oder aendern,
 * bevor Moodle-Schreibzugriffe erfolgen.
 *
 * @param {object} plan
 * @param {{ name?: string, courseName?: string, projectName?: string, topicName?: string, contentName?: string, structure?: string[] }} options
 */
function setQuestionBank(plan, options = {}) {
  const name = buildQuestionBankName(plan, options);
  const scope = normalizeQuestionBankPart(options.projectName) ? 'project' : 'course';
  const structure = Array.isArray(options.structure) && options.structure.length > 0
    ? [...options.structure]
    : ['Unterthema', 'nummerierter Inhaltsabschnitt'];
...
/**
 * Leitet den Quiz-Modus ab. Default ist "lernstandscheck"; jeder andere Modus
 * ist eine Planabweichung vom Lernstandscheck-Default.
 */
function resolveQuizMode(input) {
  ...
  const rawMode = input.mode || 'lernstandscheck';
  const mode = deprecatedAliases[rawMode] || rawMode;

  if (mode !== 'lernstandscheck') {
...
function addActivity(plan, sectionnum, activityInput) {
  ...
  // mode (mini-check/lernstandscheck/abschlusstest), Bestehensgrenze, Zeitlimit,
...
function getToolGapForActivity(activity, section) {
  const toolSupport = lookupActivitySupport(activity.type);
  if (!toolSupport.known || toolSupport.apiSupported) {
    return null;
  }
  ...
  return {
    ...
    activityLabel: toolSupport.label,
    sectionnum: section.sectionnum,
    sectionName: section.name,
    manualSteps: toolSupport.manualSteps,
  };
}
...
function getOverview(plan) {
  ...
  return {
    ...
    questionBank: plan.questionBank ? { ...plan.questionBank, structure: [...plan.questionBank.structure] } : null,
    sections: plan.sections.map(section => ({
      sectionnum: section.sectionnum,
      name: section.name,
      activities: section.activities.map(activity => {
        const toolGap = getToolGapForActivity(activity, section);
        if (toolGap) {
          toolGaps.push(toolGap);
        }
 ...
 */
async function applyPlan(plan, { approved, client, startAfterActivityId } = {}) {
  if (!approved) {
    throw new Error(
  ...
  if (startAfterActivityId && !findActivityById(plan, startAfterActivityId)) {
    throw new Error(`Wiederaufsetzpunkt "${startAfterActivityId}" ist im Plan nicht vorhanden.`);
  }

  const created = [];
  let skippingUntilResumePoint = Boolean(startAfterActivityId);

  for (const section of plan.sections) {
    for (const activity of section.activities) {
      if (skippingUntilResumePoint) {
        if (activity.id === startAfterActivityId) {
          skippingUntilResumePoint = false;
        }
        continue;
      }
      ...
      try {
        ...
        created.push({ activityId: activity.id, name: activity.name, cmid, link: result && result.link });
        ...
        error.partialResult = {
          created: [...created],
          materialGaps: [...plan.materialGaps],
          failedActivity: { activityId: activity.id, name: activity.name },
        };
        throw error;
      }
    }
