...
if (!MOODLE_URL || !MOODLE_TOKEN) {
  process.stderr.write(
    ...
    "  node moodle-mcp-quiz.js https://moodle.example.de/moodle DEIN_TOKEN\n"
  );
  process.exit(1);
}
...
function toolsForProfile() {
  if (!isReadOnlyProfile()) return QUIZ_TOOLS;
  return QUIZ_TOOLS.filter(tool => QUIZ_READ_ONLY_TOOL_NAMES.has(tool.name));
}
...
async function executeTool(name, args) {
  if (isReadOnlyProfile() && !QUIZ_READ_ONLY_TOOL_NAMES.has(name)) {
