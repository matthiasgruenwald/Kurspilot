...
function installKurspilotSkillsAliasForClaude(
  ...
  // Integritaet pruefen: echte Ordner (oder falsch zeigender Link) → Konflikt
  const conflictDirs = aliases
    .filter(({ canonicalPath, linkPath }) => {
      const status = checkAliasIntegrity(linkPath, canonicalPath);
      return status === 'not-a-link' || status === 'wrong-target';
    })
    .map(a => a.dirName);
