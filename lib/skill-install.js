 ...
 *
 * Ziel (nutzerweit), je Anbieter unter ~/.claude/skills/ bzw.
 * ~/.agents/skills/ (kanonische Codex-Ablage). ~/.codex/skills/ ist nur ein
 * vorsichtig bereinigter Legacy-Pfad:
...
/**
 * Prüft, ob Claude die gesamte verwaltete Kurspilot-Ablage über gültige
 * Aliase aus der kanonischen Codex-Ablage bezieht.
 */
function areKurspilotSkillsAliased(canonicalRoot, claudeRoot) {
  return ALIAS_DIRS.every(dirName =>
    checkAliasIntegrity(path.join(claudeRoot, dirName), path.join(canonicalRoot, dirName)) === 'ok'
  );
}
