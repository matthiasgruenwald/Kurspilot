...
const QUIZ_MCP_METADATA = Object.freeze({
  id: 'quiz',
  label: 'Quiz',
...
const QUIZ_OVERRIDE_PROPERTIES = {
  preferredbehaviour: { type: "string", description: "Frageverhalten, z.B. 'immediatefeedback', 'deferredfeedback', 'deferredcbm'. '' = Modus-Default verwenden.", default: "" },
...
const QUIZ_TOOLS = [
  {
    ...
    description: "Erstellt ein Quiz (mod_quiz) in einem Kursabschnitt. Modus wählt eine komplette Kurspilot-Settings-Kombination: 'mini-check' (kurzer Kompetenzcheck, direkte Auswertung ohne Selbsteinschätzung), 'lernstandscheck' (Default, spätere Auswertung mit Selbsteinschätzung und Lernplanung) oder 'abschlusstest' (Abschlusstest mit Verbesserungsmöglichkeit, keine Klassenarbeit). Alle weiteren Felder sind optional und überschreiben gezielt den jeweiligen Modus-Default. Fragen müssen anschließend separat zum Quiz hinzugefügt werden.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description: "Patcht ein bestehendes Quiz (mod_quiz): Nur explizit übergebene Felder werden geändert, alle anderen bleiben unverändert (reines Patch). mode wendet optional eine komplette Kurspilot-Settings-Kombination an (Frageverhalten, Layout, Navigation, Versuche, Wartezeiten, Bewertungsmethode, Review-Optionen, Gesamtfeedback, Bestehensgrenze, Abschlussbedingungen); ohne mode bleiben diese Felder unangetastet, sofern nicht einzeln überschrieben. name ändert ausschließlich den sichtbaren Titel. intro aktualisiert die Aktivitätsbeschreibung (HTML); ohne intro bleibt die bestehende Beschreibung unverändert.",
    inputSchema: {
      type: "object",
      properties: {
        cmid:      { type: "number", description: "Course Module ID des Quiz" },
        name:      { type: "string", description: "Neuer Titel des Quiz. Nicht angeben = Titel bleibt unverändert." },
        intro:     { type: "string", description: "Neue Beschreibung/Anleitung des Quiz (HTML). Nicht angeben = Beschreibung bleibt unverändert." },
        visible:   { type: "number", description: "1 = sichtbar, 0 = versteckt. Nicht angeben = Sichtbarkeit bleibt unverändert.", default: -1 },
        mode:      { type: "string", enum: ["mini-check", "lernstandscheck", "abschlusstest"], description: "Quizmodus. Nicht angeben = kein Moduswechsel; Frageverhalten, Versuche, Bewertungsmethode, Review-Optionen etc. bleiben unverändert (reines Patch der einzeln übergebenen Felder)." },
        gradepass: { type: "number", description: "Bestehensgrenze in Prozent (0-100). Nicht angeben = unverändert.", default: -1 },
        timelimit: { type: "number", description: "Zeitlimit in Sekunden (0 = unbegrenzt). Nicht angeben = unverändert.", default: -1 },
        timeopen:  { type: "number", description: "Öffnungszeitpunkt als Unix-Timestamp (0 = kein Limit). Nicht angeben = unverändert.", default: -1 },
        timeclose: { type: "number", description: "Schließzeitpunkt als Unix-Timestamp (0 = kein Limit). Nicht angeben = unverändert.", default: -1 },
        ...QUIZ_OVERRIDE_PROPERTIES,
      },
      required: ["cmid"],
    },
    async handler(args, callMoodle) {
      return await callMoodle("local_coursepilot_update_quiz_settings", {
        ...
        timeopen:  args.timeopen ?? -1,
        timeclose: args.timeclose ?? -1,
  ...
  {
    name: "moodle_plan_quiz_cleanup",
    description: "Plant eine manuelle Bereinigung, wenn eine neue Quizversion weniger Fragen enthält. Kurspilot löscht weder Quiz-Slots noch Fragen: Die Antwort nennt jeden betroffenen Slot, Frage und Kategorie sowie den direkten Moodle-Link. Dort nur aus dem Quiz entfernen, nicht aus der Fragensammlung löschen; die Fragen bleiben wiederverwendbar.",
    inputSchema: {
      type: "object",
      properties: {
        cmid: { type: "number", description: "Course module ID des Quiz" },
        keep_questionbankentryids: { type: "array", items: { type: "number" }, description: "questionbankentryid-Werte, die in der neuen Quizversion verbleiben; alle anderen Slots werden ausschließlich als manuelle Schritte ausgegeben." },
      },
      required: ["cmid", "keep_questionbankentryids"],
    },
    readOnly: true,
    async handler(args, callMoodle) {
      return await callMoodle("local_coursepilot_get_quiz_cleanup_plan", {
        cmid: args.cmid,
        ...Object.fromEntries(args.keep_questionbankentryids.map((id, i) => [`keep_questionbankentryids[${i}]`, id])),
      });
    },
  },
  {
...
const QUIZ_READ_ONLY_TOOL_NAMES = new Set(
  QUIZ_TOOLS.filter(tool => tool.readOnly).map(tool => tool.name)
);
...
async function executeQuizTool(callMoodle, name, args) {
  return await dispatchTool(QUIZ_TOOLS, name, args, callMoodle, `Unbekanntes Quiz-Tool: ${name}`);
}
