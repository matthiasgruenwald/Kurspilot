...
/**
 * Erzeugt/mergt die nutzerweiten Codex-, Claude- und opencode-MCP-
 * Konfigurationen fuer das Kurspilot-Paket (Issue #65, opencode: Issue #180,
 * ADR 0009). Schreibt ausschliesslich die dokumentierten Speicherorte aus
 * README.md ("Claude Desktop konfigurieren", "Codex konfigurieren") bzw. die
 * globale opencode-Config:
 *
 *   - Claude Desktop: claude_desktop_config.json (JSON, "mcpServers"-Objekt)
 *   - Codex:          ~/.codex/config.toml ([mcp_servers.NAME]-Tabellen)
 *   - opencode:       ~/.config/opencode/opencode.json (JSON, "mcp"-Objekt)
 *
 * Alle Eintraege rufen ausschliesslich scripts/start-mcp.js auf (siehe
 ...
 *
 * Der opencode-Writer laesst Provider-/Secret-Felder unberuehrt und fasst
 * keine Projekt-Config an (Secrets liegen ausserhalb, Spezifikation 0002).
 *
 * Bewusst kein generisches Config-Management-Framework: nur die drei
...
// --- opencode (JSON, globale opencode.json) ----------------------------------

/**
 * Baut die Kurspilot-mcp-Eintraege fuer opencode im opencode-Format
 * (type/command/enabled, siehe https://opencode.ai/docs/mcp-servers/).
 */
function buildOpenCodeEntries(startMcpPath, nodeExecPath, options = {}) {
  const serverIds = getSelectedServerIds(options.selectedActivityIds);
  const entries = {};
  for (const serverId of serverIds) {
    entries[`kurspilot-${serverId}`] = {
      type: 'local',
      command: [nodeExecPath, startMcpPath, '--server', serverId],
      enabled: true,
    };
  }
  return entries;
...
/**
 * Erzeugt oder mergt die globale opencode.json mit den Kurspilot-Eintraegen
 * (Issue #180, ADR 0009). Fremde Top-Level-Keys, fremde mcp-Eintraege und
 * Provider-/Secret-Felder bleiben unveraendert erhalten. Vor dem
 * Ueberschreiben einer vorhandenen Datei wird ein Backup angelegt.
 */
function setupOpenCodeConfig(configPath, startMcpPath, nodeExecPath, options = {}) {
  const created = !fs.existsSync(configPath);
  const backupPath = created ? null : backupExistingFile(configPath);
  ...
  for (const entryName of listAllManagedEntryNames()) {
    delete config.mcp[entryName];
  }

  config.mcp = { ...config.mcp, ...buildOpenCodeEntries(startMcpPath, nodeExecPath, options) };
...
/**
 * Escaped Backslash und Anfuehrungszeichen fuer TOML-Basic-Strings. Ohne dies
 * werden Windows-Pfade (z.B. "C:\Users\...") als ungueltige Escape-Sequenzen
 * (\U, \A, ...) interpretiert und config.toml wird fuer Codex unlesbar.
 */
...
function buildCodexEntryBlock(entryName, startMcpPath, nodeExecPath, serverId) {
  return [
    ...
    `command = "${tomlEscape(nodeExecPath)}"`,
    `args = ["${tomlEscape(startMcpPath)}", "--server", "${serverId}"]`,
...
/**
 * Entfernt die Kurspilot-mcp-Eintraege wieder aus der globalen opencode.json.
 * Fremde mcp-Eintraege, Top-Level-Keys und Provider-/Secret-Felder bleiben
 * erhalten. No-Op, wenn die Datei nicht existiert. Fuer den Uninstall-Flow.
 */
function removeKurspilotEntriesFromOpenCodeConfig(configPath) {
  if (!fs.existsSync(configPath)) {
    return { removed: false, backupPath: null, configPath };
  }
...
/**
 * Liest die zuletzt geschriebene Aktivitaets-Auswahl aus einer vorhandenen
 * Codex- oder Claude-Desktop-Config zurueck (Issue #96-Folgefehler:
 * Wartungs-/Reparaturlauf zeigte sonst immer wieder das Default-Buendel an,
 * auch wenn vorher gezielt abgewaehlt wurde). Codex hat Vorrang, weil beide
 * Clients beim Setup dieselbe Auswahl bekommen - ein Konflikt zwischen
 * beiden Dateien wird hier bewusst nicht aufgeloest, sondern die erste
 * gefundene Quelle gewinnt (karpathy-guidelines: kein Merge-Algorithmus fuer
 * einen Fall, der durch den gemeinsamen Schreibpfad gar nicht entstehen
 * sollte).
 */
function extractActivityIdsFromEntryNames(entryNames) {
  const validIds = new Set(listActivities().map((activity) => activity.id));
  return entryNames
    .filter((name) => name.startsWith('kurspilot-') && name !== `kurspilot-${CORE_SERVER}`)
    .map((name) => name.slice('kurspilot-'.length))
    .filter((id) => validIds.has(id));
}

function readConfiguredActivityIdsFromCodexConfig(configPath) {
  if (!fs.existsSync(configPath)) {
    return null;
  }
  const tomlText = fs.readFileSync(configPath, 'utf8');
  const entryNames = [...tomlText.matchAll(/^\[mcp_servers\.([A-Za-z0-9_-]+)\]\s*$/gm)].map((match) => match[1]);
  if (!entryNames.includes(`kurspilot-${CORE_SERVER}`)) {
    return null;
  }
  return extractActivityIdsFromEntryNames(entryNames);
}

function readConfiguredActivityIdsFromClaudeConfig(configPath) {
  if (!fs.existsSync(configPath)) {
    return null;
  }
  let config;
  ...
  const entryNames = Object.keys(config.mcpServers || {});
  if (!entryNames.includes(`kurspilot-${CORE_SERVER}`)) {
    return null;
  }
  return extractActivityIdsFromEntryNames(entryNames);
...
function readConfiguredActivityIds({ codexConfigPath, claudeDesktopConfigPath }) {
  const fromCodex = readConfiguredActivityIdsFromCodexConfig(codexConfigPath);
  if (fromCodex !== null) {
    return fromCodex;
  }
  return readConfiguredActivityIdsFromClaudeConfig(claudeDesktopConfigPath);
}
