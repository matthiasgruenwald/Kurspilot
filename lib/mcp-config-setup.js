...
/**
 * Erzeugt/mergt die nutzerweiten Codex- und Claude/Cowork-MCP-Konfigurationen
...
const PROFILES = [
  { entryName: 'kurspilot-planung', profile: 'readonly' },
  { entryName: 'kurspilot-umsetzung', profile: 'full' },
];

/**
 * Baut die zwei Kurspilot-MCP-Eintraege (Planung/Umsetzung), die beide den
 * tokenfreien Wrapper scripts/start-mcp.js aufrufen.
 */
function buildKurspilotEntries(startMcpPath, nodeExecPath) {
  const entries = {};
  for (const { entryName, profile } of PROFILES) {
    entries[entryName] = {
      command: nodeExecPath,
      args: [startMcpPath, '--profile', profile],
    };
  }
  return entries;
}
 ...
 */
function setupClaudeDesktopConfig(configPath, startMcpPath, nodeExecPath) {
  ...
  const kurspilotEntries = buildKurspilotEntries(startMcpPath, nodeExecPath);
...
function buildCodexEntryBlock(entryName, startMcpPath, nodeExecPath, profile) {
  return [
...
function buildCodexBlocks(startMcpPath, nodeExecPath) {
  return PROFILES.map(({ entryName, profile }) =>
    buildCodexEntryBlock(entryName, startMcpPath, nodeExecPath, profile)
  ).join('\n');
}
 ...
 */
function setupCodexConfig(configPath, startMcpPath, nodeExecPath) {
  const created = !fs.existsSync(configPath);
  const backupPath = created ? null : backupExistingFile(configPath);

  let tomlText = created ? '' : fs.readFileSync(configPath, 'utf8');

  for (const { entryName } of PROFILES) {
    tomlText = removeTomlTable(tomlText, entryName);
  }

  const trimmed = tomlText.replace(/\s+$/, '');
  const newBlocks = buildCodexBlocks(startMcpPath, nodeExecPath);
