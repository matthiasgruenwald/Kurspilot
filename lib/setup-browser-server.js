...
// Nach einem Update läuft der Dienst noch mit altem Code. Der Neustart startet
// eine frische Instanz (scripts/setup-kurspilot.js) und wartet, bis deren
// Runtime-State-Datei die neue URL verrät; solange gilt es, die offene Antwort
// nicht durch closeAllConnections() abzuschneiden.
const RESTART_DISCOVERY_TIMEOUT_MS = 15000;
...
// Startet eine frische Instanz des Konfigurationsdienstes aus dem (gerade
// aktualisierten) App-Verzeichnis. KURSPILOT_NO_BROWSER=1 verhindert, dass die
// neue Instanz einen weiteren Browser-Tab öffnet - der offene Tab wechselt
// selbst zur neuen URL (POST /restart-service liefert sie).
function defaultSpawnRestartChild(entryScriptPath) {
  const child = spawn(process.execPath, [entryScriptPath], {
    detached: true,
    stdio: 'ignore',
    env: { ...process.env, KURSPILOT_NO_BROWSER: '1' },
  });
  child.on('error', () => {});
  child.unref();
  return child;
...
function startSetupBrowserServer(options = {}) {
  const {
    ...
    spawnRestartChild = defaultSpawnRestartChild,
    restartDiscoveryTimeoutMs = RESTART_DISCOVERY_TIMEOUT_MS,
  ...
  // Nach einem erfolgreichen Neustart-Handover gehört die Runtime-State-Datei
  // der neuen Instanz - close() darf sie dann nicht mehr entfernen.
  let restartHandover = false;
  ...
  function sendCardSuccess(res, newStatus, restartRequired = []) {
    const progress = computeSetupProgress(newStatus);
    if (progress.done < progress.total) {
      wasIncompleteDuringSession = true;
    }
    ...
    res.end(JSON.stringify({
      ...
      newStatus,
      progress,
      nextCondition: nextOpenCondition(newStatus),
      wasIncomplete: wasIncompleteDuringSession,
    }));
  }
  ...
  function applyMoodle(req, res) {
    readRequestBody(req).then(body => {
      const form = new URLSearchParams(body);
      const moodleToken = formValue(form, 'moodleToken');
      // Issue #242: Token-Feld ist nie vorausgefuellt (#-Zugangsdaten-Karte),
      // ein leeres Feld bedeutet "unveraendert lassen", nicht "Token loeschen".
      const selectedMaintenanceAreaIds = moodleToken
        ? ['moodle-url-change', 'moodle-token-renewal']
        : ['moodle-url-change'];
      ...
      // Issue #245: Spec 0005/0007 nahmen an, der MCP-Server lese die
      // Zugangsdaten pro Anfrage aus dem Schlüsselbund - tatsaechlich liest
      // scripts/start-mcp.js sie einmal beim Start des MCP-Kindprozesses.
      // Laufende Codex-/Claude-Prozesse behalten die alte URL/den alten Token
      // also bis zum Neustart; die Card muss den gleichen Neustart-Hinweis
      // zeigen wie die Clients-/Aktivitaten-Cards.
      ...
      sendCardSuccess(res, newStatus, restartRequired);
    }).catch(error => {
      res.writeHead(500, { 'content-type': 'application/json; charset=utf-8' });
    ...
    Promise.resolve(applyAppUpdate(updateOptions.appUpdateOptions))
      .then(async appResult => {
        ...
        res.end(JSON.stringify({
          ...
          updated: Boolean(appResult.updated),
  ...
  // Nach einem Update läuft der Dienst noch mit dem alten Code. Der Neustart
  // löst sich aus der Runtime-State-Datei (damit die frische Instanz ihn nicht
  // ihrerseits per /abort beendet, während diese Antwort noch offen ist),
  // startet scripts/setup-kurspilot.js als abgelösten Kindprozess mit
  // KURSPILOT_NO_BROWSER=1 und wartet, bis die neue Instanz ihre eigene
  // Runtime-State-Datei geschrieben hat. Deren URL geht an den offenen Tab,
  // der daraufhin dorthin wechselt; danach beendet sich dieser Dienst.
  function handleRestartService(req, res) {
    const discoveryPath = runtimeStatePath || defaultRuntimeStatePath();
    if (runtimeStatePath) {
      removeRuntimeState(runtimeStatePath);
    }
    try {
      spawnRestartChild(path.join(__dirname, '..', 'scripts', 'setup-kurspilot.js'));
    } catch (error) {
      if (runtimeStatePath) {
      ...
      res.writeHead(500, { 'content-type': 'application/json; charset=utf-8' });
      res.end(JSON.stringify({ ok: false, error: `Neustart nicht möglich: ${error.message}` }));
      return;
    }

    const startedAt = Date.now();
    function pollNewInstance() {
      const state = readRuntimeState(discoveryPath);
      if (state && state.pid !== process.pid) {
        restartHandover = true;
        const newUrl = `http://${LOCAL_HOST}:${state.port}/?token=${state.token}`;
        finishAndClose(res, 200, 'application/json; charset=utf-8', JSON.stringify({ ok: true, url: newUrl }), close);
        return;
      }
      if (Date.now() - startedAt >= restartDiscoveryTimeoutMs) {
        // Die neue Instanz ist nicht erschienen: Dieser Dienst bleibt laufen
        // und meldet sich wieder in der Runtime-State-Datei an.
        if (runtimeStatePath) {
          writeRuntimeState(runtimeStatePath, {
            pid: process.pid,
            port: server.address().port,
        ...
        res.writeHead(200, { 'content-type': 'application/json; charset=utf-8' });
        res.end(JSON.stringify({ ok: false, error: 'Der Neustart hat nicht geklappt. Der Dienst läuft weiter - bitte versuchen Sie es erneut.' }));
        return;
      }
      setTimeout(pollNewInstance, restartPollIntervalMs);
    }
    pollNewInstance();
  }

  function handleAbort(req, res) {
    finishAndClose(res, 200, 'application/json; charset=utf-8', JSON.stringify({ ok: true }), close);
  }
  ...
  const ROUTES = [
    ...
    { method: 'POST', path: '/restart-service', handler: handleRestartService, requireToken: true },
  ];
  ...
  function close(reason = 'manual') {
    ...
    closeStarted = true;
    clearTimeout(idleTimer);
    if (runtimeStatePath && !restartHandover) {
      removeRuntimeState(runtimeStatePath);
    }
