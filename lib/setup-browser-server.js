...
function startSetupBrowserServer(options = {}) {
  ...
  const server = http.createServer((req, res) => {
    ...
    const tokenValid = requestUrl.searchParams.get('token') === token;
    ...
    if (req.method === 'GET' && pathname === TOKEN_HELP_ASSET_URL) {
    ...
    if (req.method === 'GET' && pathname === '/choose-workspace') {
    ...
    if (req.method === 'POST' && (pathname === '/end-now' || pathname === '/skip')) {
      if (!tokenValid) {
        rejectMissingToken(res);
        return;
      }
      readRequestBody(req).then(body => {
        ...
        const action = pathname === '/end-now' ? endClientHandlers[client] : () => true;
    ...
    if (req.method === 'POST' && pathname === '/finish-setup') {
      if (!tokenValid) {
        rejectMissingToken(res);
        return;
      }
    ...
    if (req.method === 'GET' && pathname === '/check-updates') {
    ...
    if (req.method === 'POST' && pathname === '/apply-updates') {
      if (!tokenValid) {
        rejectMissingToken(res);
        return;
      }
