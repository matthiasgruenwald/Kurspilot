...
function renderCurrentStateAndChanges(status, selection) {
  ...
  const cropBackendArea = areaById.get('crop-backend-prefer-imagemagick');
  const cropBackendRow = renderCropBackendSwitchRow(cropBackendArea, preselected);
...
/**
 * Schalter "ImageMagick statt sips" (Issue #139): nur sichtbar, wenn beide
 * Backends auf diesem System installiert sind - sonst gibt es nichts zu
 * waehlen. Jederzeit umschaltbar (kein Einbahnpfad), da es kein einmaliger
 * Installationsschritt ist, sondern eine dauerhafte Einstellung.
 */
function renderCropBackendSwitchRow(cropBackendArea, preselected) {
  if (!cropBackendArea) {
    return '';
  }
  const isPreferringImageMagick = preselected.has('crop-backend-prefer-imagemagick');
  const currentStatus = isPreferringImageMagick ? 'ImageMagick wird verwendet' : 'sips wird verwendet (Standard)';

  return (
    '    <article class="maintenance-row">\n' +
    '      <div>\n' +
    `        ${renderChangeCheckbox('crop-backend-prefer-imagemagick', cropBackendArea.label, preselected)}\n` +
    `        <p data-change-status data-current="${currentStatus}" data-selected="ImageMagick wird verwendet">${currentStatus}</p>\n` +
    '      </div>\n' +
    '      <div>\n' +
    '        <p>Beide Tools sind installiert - jederzeit umschaltbar, falls eines der beiden bei einem Bild Probleme macht.</p>\n' +
    '      </div>\n' +
    '    </article>\n'
  );
}
