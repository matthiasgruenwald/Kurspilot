...
function renderClientReloadSection(client) {
  ...
  return (
    ...
    `muss ${escapeHtml(label)} einmal neu gestartet werden - jetzt beenden (danach von Hand neu oeffnen) oder spaeter von Hand.</p>\n` +
...
function renderPostSaveActionsPage(report) {
  ...
  const actionsSection = pendingClients.length > 0
    ? '<section aria-labelledby="post-save-heading">\n' +
      ...
      '</section>\n'
    : '';
  const finishButton = pendingClients.length > 0
    ? ''
  ...
  return (
    ...
    '        if (result.done) {\n' +
    '          document.getElementById("post-save-final-status").textContent = "Fertig. Sie können diesen Tab jetzt schließen.";\n' +
    '        }\n' +
    '      }\n' +
    ...
    if (req.method === 'POST' && req.url === '/done') {
      readRequestBody(req).then(body => {
        ...
        if (pendingPostSaveClients.size === 0) {
          finishAndClose(res, 200, 'text/plain; charset=utf-8', `${summaryToText(report)}\n${CLOSE_TAB_HINT}\n`, close);
          return;
        }
