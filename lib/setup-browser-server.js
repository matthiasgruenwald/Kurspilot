...
function renderStatusItems(status) {
  ...
  } else if (status.imageMagick && status.imageMagick.supported) {
 ...
 * (Issue #136): positive Standard-Darstellung statt Alarm-Ton, mit den
 * bekannten sips-Einschraenkungen (Issue #135) in einfacher Sprache.
 */
function renderSipsStatusNote(status) {
  ...
  return (
    '      <p>Das eingebaute macOS-Tool (sips) schneidet Bilder zuverlässig zu - eine zusätzliche ' +
    'Installation ist dafür nicht nötig. Zwei Sonderfälle: bei animierten GIFs wird nur das erste Bild ' +
    'verwendet, und bei manchen CMYK-JPEGs ohne Farbprofil können die Farben vertauscht wirken. ' +
    'Betrifft das einen Kurs, hilft die optionale ImageMagick-Installation weiter unten.</p>\n'
  );
}
 ...
 * (Issue #136). Feste, dokumentierte Schaetzwerte statt einer Live-Messung -
 * die tatsaechliche Installation (Homebrew + "brew install imagemagick")
 * kommt erst in Issue #137, das laut Issue-Beschreibung explizit reale Werte
 * nachziehen darf, sobald die Brew-Installation existiert. Homebrew selbst
 * wird nur dann als Posten gezeigt, wenn es noch fehlt.
 */
 ...
 * bewusst NICHT geprueft - das ist Teil der tatsaechlichen Installation in
 * Issue #137 (method: 'brew'). Bis dahin wird der Homebrew-Posten als
 * "falls noch nicht vorhanden" ausgewiesen, damit die Lehrkraft vorab den
 * realistischen Maximalverbrauch sieht statt einer zu niedrigen Schaetzung.
 */
...
function renderImageMagickMaintenanceRow(status, imageMagickArea, preselected) {
  ...
  if (status.imageMagick && status.imageMagick.sipsActive) {
    return (
      ...
      '      <div>\n' +
      `        ${renderChangeCheckbox('imagemagick-install', 'Nur bei diesen Problemen: ImageMagick installieren', preselected)}\n` +
...
/**
 * Seite nach dem Speichern (Issue #130): Config wird immer geschrieben,
 * unabhaengig vom Claude-Laufzeitstatus (siehe Race-Condition-Analyse in
 * lib/setup-flow.js). Lief Claude waehrend des Speicherns, bietet diese
 * Seite optionale Folgeaktionen an - Opt-in statt Zwang, an die menschliche
 * Nutzerin gerichtet: "jetzt beenden", "jetzt neu starten" oder einfach
 * "Fertig" (manuell spaeter selbst neu starten). Codex hat keinen
 * vergleichbaren Dauerprozess (CLI-Tool, kein Hintergrunddienst) - deshalb
 * gibt es hier bewusst keine Codex-Beenden/Neustart-Option.
 */
function renderPostSaveActionsPage(report) {
  const summaryText = summaryToText(report);
  const actionsSection = report.claudeWasRunningDuringSave
    ? '<section aria-labelledby="post-save-heading">\n' +
      '  <h2 id="post-save-heading">Claude neu laden</h2>\n' +
      '  <p>Claude lief beim Speichern noch. Damit die neue Konfiguration sicher geladen wird, ' +
      'muss Claude einmal neu gestartet werden - jetzt oder später von Hand.</p>\n' +
      '  <button id="end-claude-now-button" type="button">Claude jetzt beenden</button>\n' +
      '  <button id="restart-claude-now-button" type="button">Claude jetzt neu starten</button>\n' +
      '  <button id="finish-without-action-button" type="button">Später von Hand</button>\n' +
      '  <p id="post-save-status" role="status" aria-live="polite"></p>\n' +
      '</section>\n'
    : '';
  const finishButton = report.claudeWasRunningDuringSave
    ? ''
    : '<form method="post" action="/finish-setup"><button type="submit">Fertig</button></form>\n';
  ...
  return (
    ...
    finishButton +
    '    <script>\n' +
    '      document.getElementById("end-claude-now-button")?.addEventListener("click", async event => {\n' +
    '        const button = event.currentTarget;\n' +
    '        button.disabled = true;\n' +
    '        document.getElementById("post-save-status").textContent = "Claude wird beendet...";\n' +
    '        await fetch("/end-claude-now", { method: "POST" });\n' +
    '        document.getElementById("post-save-status").textContent = "Claude wurde beendet. Fertig.";\n' +
    '      });\n' +
    '      document.getElementById("restart-claude-now-button")?.addEventListener("click", async event => {\n' +
    '        const button = event.currentTarget;\n' +
    '        button.disabled = true;\n' +
    '        document.getElementById("post-save-status").textContent = "Claude wird neu gestartet...";\n' +
    '        await fetch("/restart-claude-now", { method: "POST" });\n' +
    '        document.getElementById("post-save-status").textContent = "Claude wurde neu gestartet. Fertig.";\n' +
    '      });\n' +
    '      document.getElementById("finish-without-action-button")?.addEventListener("click", async () => {\n' +
    '        await fetch("/finish-setup", { method: "POST" });\n' +
    '        document.getElementById("post-save-status").textContent = "Fertig.";\n' +
    '      });\n' +
    '    </script>\n' +
    '  </main>\n' +
    '</body>\n' +
...
function startSetupBrowserServer(options = {}) {
  const {
    ...
    restartClaudeDesktop = defaultRestartClaudeDesktop,
  ...
  const server = http.createServer((req, res) => {
    ...
    if (req.method === 'POST' && req.url === '/done') {
      readRequestBody(req).then(body => {
        ...
        // Issue #130: blockierte Faelle (kein Client erkannt) und der
        // Normalfall (Claude lief nicht beim Speichern) schliessen den
        // Dienst weiterhin sofort. Lief Claude beim Speichern, bleibt der
        // Dienst offen, damit die Lehrkraft optional "Beenden"/"Neustart"
        // anklicken kann (Opt-in, siehe renderPostSaveActionsPage).
        if (report.blocked) {
          finishAndClose(res, 400, 'text/plain; charset=utf-8', summaryToText(report), close);
          return;
        }
        if (!report.claudeWasRunningDuringSave) {
          finishAndClose(res, 200, 'text/plain; charset=utf-8', summaryToText(report), close);
          return;
        }
    ...
    if (req.method === 'POST' && req.url === '/end-claude-now') {
      Promise.resolve(endClaudeDesktop()).then(() => {
    ...
    if (req.method === 'POST' && req.url === '/restart-claude-now') {
      Promise.resolve(restartClaudeDesktop({ isClaudeRunning })).then(() => {
        res.writeHead(200, { 'content-type': 'text/plain; charset=utf-8' });
        res.end('Claude wurde neu gestartet.\n', () => {
          close();
        });
      });
      return;
