 ...
 * lib/activity-registry.js gepflegten Aktivitaeten als Checkboxen, gaengige
 * default an, exotische default aus - Core ist immer aktiv und wird hier
 * bewusst nicht aufgelistet (siehe lib/mcp-config-setup.js, CORE_SERVER).
 * Reichen das Setup-Tool unveraendert per "activity"-Feldern weiter; die
 * Abhaengigkeitsaufloesung (z.B. Quiz -> Fragensammlung) passiert bereits in
 * lib/mcp-config-setup.js und wird hier nicht dupliziert.
 *
 * ponytail: zeigt bei jedem Aufruf das Default-Bundle vorausgewaehlt, liest
 * keine zuvor konfigurierte Auswahl aus den bestehenden Config-Dateien
 * zurueck. Bei Reparatur eines Setups mit individueller Auswahl muss die
 * Lehrkraft die Checkliste also erneut anpassen. Upgrade-Pfad falls das
 * stoert: Auswahl aus den vorhandenen Config-Eintraegen (kurspilot-<id>)
 * rekonstruieren statt getDefaultBundle() ungeprueft als Default zu zeigen.
 */
function renderActivityChecklist(disabled = false) {
  const disabledAttr = disabled ? ' disabled' : '';
  const items = listActivities()
    .map(activity => {
      const checked = activity.default ? ' checked' : '';
      return (
        '<li>' +
...
function renderCurrentStateAndChanges(status, selection) {
  ...
  return (
    ...
    `        ${renderActivityChecklist(!preselected.has('kurspilot-setup-or-repair'))}\n` +
    '      </div>\n' +
    '    </article>\n' +
    '    <article class="maintenance-row">\n' +
    '      <div>\n' +
