...
function summaryToText(report) {
  if (report.blocked) {
    // Issue #183: Installationslinks aller Clients (Codex, Claude, opencode)
    // direkt in die Blocker-Antwort - Spiegel von reportToText im CLI-Weg.
    const links = Object.entries(report.installLinks || {})
      .map(([client, url]) => `  - ${client}: ${url}`)
      .join('\n');
    return (
      'Kurspilot-Konfiguration wurde nicht ausgeführt: kein Client erkannt.\n' +
      (links ? `Installationslinks:\n${links}\n` : '')
    );
  }
...
function startSetupBrowserServer(options = {}) {
  ...
  // Issue #202: In-Memory-Flag, gesetzt durch POST /restart-setup. Ueberschreibt
  // die automatische Ansichtswahl in handleRoot, ohne gespeicherte Werte zu
  // aendern - die Lehrkraft kommt so aus der Wartungs-Ansicht zurueck zur
  // ausfuehrlichen Ersteinrichtung.
  let forceSetupMode = false;
  ...
  function handleRoot(req, res) {
    ...
    // Issue #231: Flag unabhaengig von der Ansichtswahl setzen - auch die
    // Ersteinrichtungs-Ansicht (heutiges Routing unter 6/6) zaehlt als
    // "war unvollstaendig". Die Wartungs-Ansicht wertet die Flag erst bei
    // erreichtem Maximum aus (Erfolgsbanner).
    if (progress.done < progress.total) {
      wasIncompleteDuringSession = true;
    }
    // Issue #202: automatische Ansichtswahl. Mindestkonfiguration erfuellt und
    // nicht bewusst in die Ersteinrichtung zurueckgeschaltet -> Wartungs-Ansicht;
    // sonst die ausfuehrliche Ersteinrichtungs-Ansicht.
    if (isMinimumConfigured(status) && !forceSetupMode) {
    ...
    const html = renderSetupPage(status, selection, { startMode, progress });
    res.writeHead(200, { 'content-type': 'text/html; charset=utf-8' });
    res.end(html);
  }

  function handleRestartSetup(req, res) {
    forceSetupMode = true;
  ...
  function handleRestartClient(req, res) {
    readRequestBody(req).then(body => {
      ...
      // endClientHandlers (Spec 0005 S5). pendingPostSaveClients bleibt unberuehrt
      // (reine Post-Save-Buchfuehrung des /done-Flows).
      if (client === 'opencode') {
        res.writeHead(200, { 'content-type': 'application/json; charset=utf-8' });
  ...
  function handleEndNow(req, res) {
    handlePostSaveClient(req, res, true);
  }

  function handleSkip(req, res) {
    handlePostSaveClient(req, res, false);
  }

  function handlePostSaveClient(req, res, endNow) {
    readRequestBody(req).then(body => {
      const form = new URLSearchParams(body);
      const client = form.get('client');
      if (client !== 'codex' && client !== 'claude') {
      ...
      const action = endNow ? endClientHandlers[client] : () => true;
  ...
  function handleFinishSetup(req, res) {
