 ...
 * Checkbox aus #139, die als Lehrkraft-Feedback unpassend wirkte: ein
 * Werkzeugwahl ist keine "anhaken = tun"-Wartungsaktion). Nur sichtbar, wenn
 * beide Backends auf diesem System installiert sind - sonst gibt es nichts zu
 * waehlen. Jederzeit umschaltbar, kein eigenes "maintenance"-Checkbox-Feld -
 * stattdessen ein dediziertes Radio-Paar (name="cropBackend"), das
 * runSetupFlow als cropBackendChoice erreicht.
 */
function renderCropBackendSwitchRow(status) {
  ...
  return (
    ...
    `          <label><input type="radio" name="cropBackend" value="sips"${sipsChecked} data-toggles-explainer> sips</label>\n` +
    `          <label><input type="radio" name="cropBackend" value="imagemagick"${imageMagickChecked} data-toggles-explainer> ImageMagick</label>\n` +
    '        </div>\n' +
    '      </div>\n' +
    '      <div>\n' +
...
function renderSetupPage(status, options = {}) {
  ...
  return (
    ...
    '    .toggle-switch label { margin: 0; padding: 0.4rem 1rem; cursor: pointer; }\n' +
    '    .toggle-switch label:has(input:checked) { background: #1f2933; color: #fff; font-weight: 600; }\n' +
