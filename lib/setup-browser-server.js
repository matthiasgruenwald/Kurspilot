...
function startSetupBrowserServer(options = {}) {
  ...
  function handleRoot(req, res) {
    ...
    const html = renderSetupPage(status, selection, { startMode, progress: computeSetupProgress(status) });
    res.writeHead(200, { 'content-type': 'text/html; charset=utf-8' });
    res.end(html);
  }
  ...
  function applyCropBackend(req, res) {
    readRequestBody(req).then(body => {
      ...
      // Karte ohne beide Backends (z.B. Windows) bietet statt der Radios einen
      // Reparatur-/Install-Button - teilt sich den Endpoint ueber dasselbe
      // Maintenance-Area-Handling wie die Ersteinrichtungs-Checkbox.
