 ...
 * app-provision.js
 *
 * Laedt den aktuellen main-Tarball von GitHub
...
const APP_TARBALL_URL = 'https://github.com/matthiasgruenwald/moodle-coursepilot/archive/refs/heads/main.tar.gz';
 ...
 * provisionApp() das Node-globale `fetch` (>=24, siehe scripts/bootstrap-
 ...
 */
async function defaultFetchBuffer(url, options) {
...
function fetchWithTimeout(fetchFn, url, timeoutMs) {
  ...
  return new Promise((resolve, reject) => {
    ...
    Promise.resolve(fetchFn(url, { signal: controller.signal })).then(
      value => {
        clearTimeout(timer);
        resolve(value);
      },
...
/**
 * Laedt den aktuellen GitHub-main-Tarball und entpackt ihn nach ~/.kurspilot/app (bzw.
 ...
 * @param {string} [options.tarballUrl] alternative Tarball-URL fuer den Update-Kanal
...
async function provisionApp(options = {}) {
  ...
  const tarballBuffer = await fetchWithTimeout(fetchFn, tarballUrl, timeoutMs);
