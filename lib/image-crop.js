...
const { resolveMagickBinary } = require('./imagemagick-setup');
 ...
 *
 * Unter Windows faellt das auf den Kurspilot-eigenen Installationsort
 * zurueck, falls `magick` nicht im PATH liegt (siehe resolveMagickBinary in
 * lib/imagemagick-setup.js, Issue #145 - sonst findet weder dieses Tool noch
 * Codex/Claude ein per Setup installiertes, PATH-loses ImageMagick).
 *
 * @returns {string} 'magick'/voller Pfad unter Windows, sonst 'convert'
 */
function getConvertBinary() {
  const platform = os.platform();
  return platform === 'win32' ? resolveMagickBinary({ platform }) : 'convert';
}
 ...
 * @param {{preferredBackend?: 'sips'|'imagemagick'}} [options] bevorzugtes Backend (Issue #139);
 *   schlaegt es fehl, wird automatisch das andere (sofern auf der Plattform nutzbar) versucht
 * @returns {{backend: 'sips'|'imagemagick'}} welches Backend tatsaechlich genutzt wurde
 * @throws {Error} wenn `sourcePath` nicht existiert, `region` ungueltig ist,
 *   oder alle versuchten Crop-Aufrufe fehlschlagen
 */
function cropImage(sourcePath, region, destPath, options = {}) {
  ...
  for (const backend of resolveBackendOrder(options.preferredBackend)) {
    try {
      return backend === 'sips'
        ? cropImageWithSips(sourcePath, { x, y, width, height }, destPath)
        : cropImageWithImageMagick(sourcePath, { x, y, width, height }, destPath);
    } catch (err) {
      lastError = err;
    }
  }
...
/**
 * Reihenfolge der zu versuchenden Backends (Issue #139: automatischer
 * Fallback, wenn das bevorzugte Backend zur Laufzeit scheitert). `sips`
 * existiert nur auf macOS - dort entscheidet die Praeferenz die Reihenfolge,
 * auf allen anderen Plattformen bleibt ImageMagick die einzige Option.
 *
 * @param {'sips'|'imagemagick'} [preferredBackend]
 * @returns {Array<'sips'|'imagemagick'>}
 */
function resolveBackendOrder(preferredBackend) {
  if (os.platform() !== 'darwin') {
    return ['imagemagick'];
  }
  return preferredBackend === 'imagemagick' ? ['imagemagick', 'sips'] : ['sips', 'imagemagick'];
}
