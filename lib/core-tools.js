 ...
 */
function inheritedSettingsNotes(source, { includeAvailabilityNote = true } = {}) {
  const notes = [];
  ...
  if (source.availability && includeAvailabilityNote) {
...
async function cloneActivityToOtherCourse(args, source, callMoodle) {
  ...
  const notes = inheritedSettingsNotes(source, { includeAvailabilityNote: false });
  if (source.availability) {
    // Moodles Backup/Restore uebertraegt availability rein strukturell und
    // kann cmid-Referenzen nicht kursuebergreifend aufloesen (die Quell-cmid
    // existiert im Zielkurs nicht) - das Ergebnis ist eine kaputte
    // Bedingung, die auf keine echte Aktivitaet mehr zeigt (#332). Statt den
    // Klon mit einer unsichtbar kaputten Sperre auszuliefern, wird sie
    // explizit entfernt.
    await callMoodle("local_coursepilot_set_restriction", { cmid: newcmid });
    notes.push("Voraussetzungen (availability) der Quelle konnten kursuebergreifend nicht sinnvoll uebertragen werden (Quell-cmid existiert im Zielkurs nicht) und wurden entfernt - bei Bedarf im Zielkurs neu setzen.");
  }
  if (manualFollowUp) {
...
const CORE_TOOLS = [
  {
    ...
    description: "Setzt Sichtbarkeit, Stealth-Modus und/oder Gruppenmodus einer bestehenden Aktivität. Die Änderung nutzt Moodles Core-Kursseiten-API; danach moodle_get_course_catalog zum frischen Read-back aufrufen. coursepagevisibility='stealth': Aktivität bleibt für Lernende über Direktlinks (z.B. Lernlandkarten) erreichbar, erscheint aber nicht in der Kursseitenliste. 'shown' stellt die Aktivität wieder auf der Kursseite dar. Nicht alle Moodle-Instanzen/Kursformate unterstützen Stealth – Read-back prüft, ob visibleoncoursepage tatsächlich 0 wurde.",
    inputSchema: {
      type: "object",
      properties: {
        courseid: { type: "number", description: "Kurs-ID" },
        cmid: { type: "number", description: "Course Module ID der Aktivität" },
        visible: { type: "number", enum: [0, 1], description: "0 = versteckt (Lernende können nicht zugreifen), 1 = sichtbar/normal. Für 'erreichbar aber nicht in Kursliste' stattdessen coursepagevisibility='stealth' verwenden." },
        coursepagevisibility: { type: "string", enum: ["shown", "stealth"], description: "'stealth' = verfügbar aber nicht auf der Kursseite anzeigen (visible=1, visibleoncoursepage=0). 'shown' = normal auf Kursseite anzeigen (visible=1, visibleoncoursepage=1). Getrennt von visible (echtes Verstecken)." },
    ...
    async handler(args, callMoodle) {
      if (args.visible === undefined && args.coursepagevisibility === undefined && args.groupmode === undefined) {
        throw new Error("moodle_update_activity_settings: visible, coursepagevisibility oder groupmode muss angegeben werden.");
      }
      if (args.visible !== undefined) {
        await setCmVisibility(callMoodle, args.courseid, args.cmid, args.visible);
      }
      if (args.coursepagevisibility !== undefined) {
        const action = args.coursepagevisibility === "stealth" ? "cm_stealth" : "cm_show";
        await callMoodle("core_courseformat_update_course", {
          action,
          courseid: args.courseid,
          "ids[0]": args.cmid,
        });
        if (args.coursepagevisibility === "stealth") {
          return { warning: "Stealth gesetzt. Read-back mit moodle_get_course_catalog prüfen: visibleoncoursepage muss 0 sein. Falls 1 zurückkommt, unterstützt diese Moodle-Instanz oder das Kursformat keinen Stealth-Modus." };
        }
      }
