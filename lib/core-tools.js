...
const CORE_TOOLS = [
  ...
  },
  {
    ...
    description: "Dupliziert eine Aktivitaet im selben Kurs (Moodle-Core cm_duplicate, Spec 0013 KP-010) inkl. Plugin-Konfigurationen (z.B. Abgabetypen bei Aufgaben), Bewertungsfeldern, Completion und Voraussetzungen. Entfernt automatisch das „(Kopie)“-Suffix bzw. setzt title, und macht den Klon sichtbar (visible=1), sofern nicht explizit visible=0 angegeben. Kursuebergreifendes Klonen (abweichende courseid) wird von diesem Tool nicht unterstuetzt. Enthaelt bereits einen Hinweis, falls Voraussetzungen oder Abschlussregeln von der Quelle uebernommen wurden - diesen Hinweis an die Lehrkraft weitergeben. Fuer Aktivitaetstypen ohne eigenes MCP-Update-Tool (nur Titel setzbar) manuelle Nacharbeit in Moodle beschreiben.",
    inputSchema: {
      type: "object",
      properties: {
        ...
        sectionnum: { type: "number", description: "Ziel-Abschnittsnummer (0-basiert). Weglassen = selber Abschnitt wie Quelle" },
        courseid:   { type: "number", description: "Zielkurs. Weglassen oder identisch mit dem Quellkurs; abweichende Werte werden abgelehnt (kursuebergreifendes Klonen nicht unterstuetzt)" },
    ...
    async handler(args, callMoodle) {
      ...
      if (args.courseid !== undefined && args.courseid !== source.course) {
        throw new Error("moodle_clone_activity: kursuebergreifendes Klonen wird von diesem Tool nicht unterstuetzt (Zielkurs muss dem Quellkurs entsprechen).");
      }
