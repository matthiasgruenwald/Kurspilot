...
function renderMaintenancePage(status) {
  return (
    ...
    renderPlaceholderCard('workspace', 'Arbeitsbereich') +
    renderPlaceholderCard('clients', 'KI-Clients') +
    renderPlaceholderCard('activities', 'Aktivitäten') +
    renderPlaceholderCard('crop-backend', 'Bildausschnitt') +
    renderPlaceholderCard('version', 'Version') +
    '    </div>\n' +
    '  </main>\n' +
    ...
    '        for (const input of detail.querySelectorAll("input")) body.set(input.name, input.value);\n' +
    '        button.disabled = true;\n' +
    ...
    '            if (summary && result.newStatus) summary.textContent = moodleSummaryText(result.newStatus.moodle);\n' +
    '            detail.hidden = true;\n' +
    '          } else {\n' +
