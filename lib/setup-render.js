...
/**
 * Option "Gemeinsame Skill-Ablage" (Issue #165): nur sichtbar wenn beide
 * Clients erkannt - JS aktualisiert die Sichtbarkeit dynamisch bei
 * Aenderung der Client-Auswahl. Lehrkraftsprache: kein "Symlink"/"Junction".
 */
function renderSharedStorageOption(status, disabled) {
  const bothDetected = Boolean(status.detectedClients.codex && status.detectedClients.claude);
  const hiddenAttr = bothDetected ? '' : ' hidden';
  const disabledAttr = disabled ? ' disabled' : '';
  return (
...
function renderSetupPage(status, selection, options = {}) {
  ...
  return (
    ...
    '        update();\n' +
    '      }\n' +
    '      // Issue #165: Gemeinsame-Ablage-Option nur sichtbar wenn beide Clients angehaekt.\n' +
    ...
    '        const versionHint = info.versionNew ? ` (${info.versionCurrent ? info.versionCurrent + " → " : ""}${info.versionNew})` : "";\n' +
    '        return `<li>${info.label}: ${info.updateAvailable ? "Update verfügbar" : "Aktuell"}${versionHint}</li>`;\n' +
    '      }\n' +
...
function renderCloseTabButton(hidden = false) {
  return `<button id="close-tab-button" type="button"${hidden ? ' hidden' : ''}>Fertig und Tab schließen</button>\n`;
}
...
function renderPostSaveActionsPage(report) {
  ...
  const actionsSection = pendingClients.length > 0
    ? '<section aria-labelledby="post-save-heading">\n' +
      ...
      `  ${renderCloseTabButton(true)}` +
      '</section>\n'
    : '';
  const finishButton = pendingClients.length > 0
    ? ''
    : renderCloseTabButton();
  ...
  return (
    ...
    '  <main>\n' +
    '    <h1>Kurspilot konfigurieren</h1>\n' +
    renderSetupResult(report, { closeHint: pendingClients.length === 0 }) +
    actionsSection +
    finishButton +
    '    <script>\n' +
    '      document.getElementById("close-tab-button")?.addEventListener("click", () => {\n' +
    '        window.close();\n' +
    `        document.getElementById("close-tab-button").textContent = "${CLOSE_TAB_HINT}";\n` +
    '      });\n' +
    ...
    '          document.getElementById("close-tab-button").hidden = false;\n' +
    '        }\n' +
    '      }\n' +
