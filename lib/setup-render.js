 ...
 * Nach einer erfolgreich installierten Änderung erscheint die Abschlussfrage
 * 'Jetzt Dienst neu starten oder beenden?' mit zwei Buttons: Neustart
 * (POST /restart-service, wechselt auf die frische Instanz) und Beenden
 * (POST /abort, wie der Footer-Button, mit Fertig-Overlay).
 */
function renderVersionCard() {
  return (
    ...
    '  <div data-card-detail="version" hidden>\n' +
    '    <div class="version-restart" data-version-restart hidden>\n' +
    '      <p class="restart-notice">Das Update ist installiert. Der Dienst läuft noch mit der alten Version - jetzt Dienst neu starten oder beenden?</p>\n' +
    '      <div class="version-restart-actions">\n' +
    '        <button class="version-restart-button btn-primary" type="button">Dienst neu starten</button>\n' +
    '        <button class="version-end-button btn-secondary" type="button">Dienst beenden</button>\n' +
    '      </div>\n' +
    '      <p class="version-restart-status" role="status" aria-live="polite"></p>\n' +
    '    </div>\n' +
    '  </div>\n' +
    '</article>\n'
  );
...
function renderMaintenanceProgress(progress, nextCondition) {
  ...
  return (
    ...
    '        </div>\n' +
    '      </div>\n' +
    `      <button class="btn-primary maintenance-progress-next" type="button" data-next-card="${escapeHtml(nextCondition.cardId)}" data-next-label="${escapeHtml(nextCondition.label)}">Weiter zu: ${escapeHtml(nextCondition.label)}</button>\n` +
    '    </div>\n'
  );
}
...
function renderMaintenancePage(status, options = {}) {
  return (
    ...
    '    .card-column.is-wide-open { grid-column: span 2; }\n' +
    ...
    '    @media (max-width: 959px) { .card-column.is-wide-open { grid-column: span 1; } .card.is-open.card-wide .card-detail { display: grid; column-count: 1; } .card.is-open.card-wide .card-detail > * + * { margin-top: 0; } }\n' +
    ...
    '    .version-restart { display: grid; gap: var(--kp-space-sm); }\n' +
    '    .version-restart-actions { display: flex; flex-wrap: wrap; gap: var(--kp-space-sm); }\n' +
    '    .version-restart-status { margin: 0; color: var(--kp-text-muted); font-size: 0.9rem; }\n' +
    '    .version-restart-status:empty { display: none; }\n' +
    ...
    '    async function endServiceAndShowOverlay(statusLine) {\n' +
    '      window.kurspilotServiceStopped = true;\n' +
    ...
    '      endServiceAndShowOverlay(document.getElementById("abort-status"));\n' +
    '    });\n' +
    ...
    '      const column = card ? card.closest(".card-column") : null;\n' +
    '      const trigger = card ? card.querySelector(".card-edit") : null;\n' +
    '      detail.hidden = !open;\n' +
    '      if (card) card.classList.toggle("is-open", open);\n' +
    '      if (column && card) column.classList.toggle("is-wide-open", open && card.classList.contains("card-wide"));\n' +
    ...
    '    function updateMaintenanceProgress(progress, nextCondition, wasIncomplete) {\n' +
    '      const progressEl = document.querySelector("[data-maintenance-progress]");\n' +
    '      if (!progressEl || !progress) return;\n' +
    '      if (progress.done >= progress.total || !nextCondition) {\n' +
    '        progressEl.remove();\n' +
    '        if (wasIncomplete && !document.querySelector("[data-maintenance-success]")) {\n' +
    '          const banner = document.createElement("div");\n' +
    '          banner.className = "maintenance-success-banner";\n' +
    '          banner.setAttribute("role", "status");\n' +
    '          banner.dataset.maintenanceSuccess = "";\n' +
    '          banner.textContent = "Kurspilot ist eingerichtet — Sie sind startklar.";\n' +
    '          document.querySelector("#maintenance-cards").before(banner);\n' +
    '        }\n' +
    '        return;\n' +
    '      }\n' +
    '      const percent = Math.round((progress.done / progress.total) * 100);\n' +
    '      progressEl.querySelector(".maintenance-progress-text").textContent = `Schritt ${progress.done} von ${progress.total} erledigt`;\n' +
    '      progressEl.querySelector(".maintenance-progress-fill").style.width = `${percent}%`;\n' +
    '      const nextButton = progressEl.querySelector(".maintenance-progress-next");\n' +
    '      nextButton.dataset.nextCard = nextCondition.cardId;\n' +
    '      nextButton.dataset.nextLabel = nextCondition.label;\n' +
    '      nextButton.textContent = `Weiter zu: ${nextCondition.label}`;\n' +
    ...
    '        nextButton.textContent = `Jetzt einrichten: ${nextButton.dataset.nextLabel}`;\n' +
    ...
    '            updateMaintenanceProgress(result.progress, result.nextCondition, result.wasIncomplete);\n' +
    ...
    '      function poll() {\n' +
    '        if (attempts++ >= 600) {\n' +
    '          statusEl.textContent = "Installation dauert zu lang – Seite neu laden.";\n' +
    '          if (button) button.disabled = false;\n' +
    '          return;\n' +
    '        }\n' +
    '        fetch(`/install-status?token=${encodeURIComponent(currentToken())}`)\n' +
    '          .then(r => r.json())\n' +
    '          .then(state => {\n' +
    ...
    '      const restartBlock = card ? card.querySelector("[data-version-restart]") : null;\n' +
    '      const restartButton = card ? card.querySelector(".version-restart-button") : null;\n' +
    '      const endButton = card ? card.querySelector(".version-end-button") : null;\n' +
    '      const restartStatus = card ? card.querySelector(".version-restart-status") : null;\n' +
    '      function showRestartChoice() {\n' +
    '        if (restartBlock) restartBlock.hidden = false;\n' +
    '      }\n' +
    '      if (restartButton) {\n' +
    '        restartButton.addEventListener("click", async () => {\n' +
    '          restartButton.disabled = true;\n' +
    '          if (endButton) endButton.disabled = true;\n' +
    '          if (restartStatus) restartStatus.textContent = "Dienst wird neu gestartet…";\n' +
    '          try {\n' +
    '            const response = await fetch(`/restart-service?token=${encodeURIComponent(currentToken())}`, { method: "POST" });\n' +
    '            const data = await response.json();\n' +
    '            if (data.ok && data.url) {\n' +
    '              if (restartStatus) restartStatus.textContent = "Seite wird neu geladen…";\n' +
    '              window.location.href = data.url;\n' +
    '              return;\n' +
    '            }\n' +
    '            if (restartStatus) restartStatus.textContent = data.error || "Neustart nicht möglich.";\n' +
    '          } catch {\n' +
    '            if (restartStatus) restartStatus.textContent = "Neustart nicht möglich.";\n' +
    '          }\n' +
    ...
    '          if (restartStatus) endServiceAndShowOverlay(restartStatus);\n' +
    '        });\n' +
    '      }\n' +
    ...
    '            summary.textContent = "Update installiert";\n' +
    '            showRestartChoice();\n' +
    '            button.textContent = "erneut prüfen";\n' +
    '            button.dataset.action = "check";\n' +
    '            return;\n' +
