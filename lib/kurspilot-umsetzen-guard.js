...
function formatImplementationPoint(item) {
  ...
  return `${labelPart}${item.name} (Plan-ID ${item.activityId}${moodleIdPart})`;
}
...
async function findImplementationPreflightConflicts(plan, catalog, { implementationPoint } = {}) {
  const conflicts = [];
...
async function fetchCourseCatalogForPreflight(plan, client) {
  ...
  return { catalog, fetchError: null };
}
...
async function runKurspilotUmsetzenGuard(workspaceRoot, { plan, client, now, updatingSkill = 'kurspilot-umsetzen' }) {
  ...
  const { catalog, fetchError } = await fetchCourseCatalogForPreflight(plan, client);
  const preflightConflicts = fetchError
    ? [fetchError]
    : await findImplementationPreflightConflicts(plan, catalog, { implementationPoint });
