...
function resolveMagickBinary(options = {}) {
  ...
  const candidate = path.join(
    getKurspilotImageMagickDir({ homeDir, platform, localAppData: options.localAppData }),
    'magick.exe'
  );
  return existsSync(candidate) ? candidate : 'magick';
}
...
function installImageMagickWindows(options = {}) {
  ...
  const psCommand = [
    ...
    `tar --strip-components=1 -xf $archivePath -C ${psQuote(destDir)};`,
