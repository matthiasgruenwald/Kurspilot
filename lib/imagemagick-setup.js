 ...
 * portablen Pakete liegen auf GitHub Releases (https://github.com/ImageMagick/
 * ImageMagick/releases) ausschliesslich als .7z vor (seit ca. 7.1.x keine
 * .zip-Variante mehr). Windows bsdtar (System32\tar.exe) unterstuetzt kein
 * LZMA und scheitert an .7z (Issue #172). Loesung: 7-Zip-Standalone (7za.exe,
 * <400KB, von 7-zip.org als .zip erhältlich) wird per WINDOWS_7ZA_BOOTSTRAP_URL
 * kurz geboostrap und dann zum Entpacken verwendet; beide Dateien werden
 * danach geloescht. Kein winget, kein Installer, keine UAC-Anfrage.
 ...
 * PowerShell-Download via Invoke-WebRequest in %LOCALAPPDATA% benoetigt unter
 * Standard-Policies kein Admin-Recht. Falls doch (z.B. restriktive Unternehmens-
 * GPO), bitte Issue mit Live-Befund neu oeffnen.
 */
...
/**
 * 7-Zip Standalone-Konsole (7za.exe) als .zip - wird als Bootstrapper
 * benoetigt, um das LZMA-komprimierte ImageMagick-.7z zu entpacken, da
 * Windows bsdtar kein LZMA kann (Issue #172). Die URL ist seit 2010 stabil
 * (Version 9.20, 32-Bit, laeuft via WOW64, kann moderne .7z-Archive lesen).
 */
const WINDOWS_7ZA_BOOTSTRAP_URL = 'https://www.7-zip.org/a/7za920.zip';
...
const IMAGEMAGICK_DOWNLOAD_TABLE = {
  ...
  // x64-Binary laeuft auf ARM64-Windows via Emulation; synchroner PowerShell-
  // Pfad wie x64 (kein async/Promise -> kein stiller Fehler in setup-flow).
  // Die alte async-Zip-URL (imagemagick.org/archive) ist zudem 404 (Issue #129).
  'win32-arm64': {
    method: 'windows-portable',
  },
  'darwin-arm64': {
...
/**
 * Loest den ausfuehrbaren magick-Befehl auf: auf macOS zuerst bekannte
 * Homebrew-Pfade (GUI-Prozesse ohne Homebrew-PATH, analog resolveBrewBinary),
 * dann 'convert' als Fallback; auf Windows Kurspilot-eigene Installation oder
 * 'magick' (PATH). Auf Linux: 'convert'.
 *
 ...
 * @returns {string} absoluter Pfad, 'magick' oder 'convert'
 */
function resolveMagickBinary(options = {}) {
  const platform = options.platform || process.platform;
  if (platform !== 'win32') {
    if (platform === 'darwin') {
      const existsSync = options.existsSync || fs.existsSync;
      const found = MAGICK_BIN_CANDIDATES_DARWIN.find(c => existsSync(c));
  ...
  const magickDir = getKurspilotImageMagickDir({ homeDir, platform, localAppData: options.localAppData });
  const direct = path.join(magickDir, 'magick.exe');
  if (existsSync(direct)) return direct;
  // Portable-Archiv entpackt in einen benannten Unterordner; einen Level tiefer suchen
  const readdirSync = options.readdirSync || fs.readdirSync;
  try {
    for (const entry of readdirSync(magickDir, { withFileTypes: true })) {
      if (!entry.isDirectory()) continue;
      const sub = path.join(magickDir, entry.name, 'magick.exe');
      if (existsSync(sub)) return sub;
    }
  } catch {
    // magickDir existiert noch nicht
  }
  return 'magick';
}
 ...
 * Faellt auf den Kurspilot-eigenen Installationsort zurueck, falls `magick`
 * nicht im PATH liegt (siehe resolveMagickBinary, Issue #145).
 *
 * @param {object} [options]
...
function isImageMagickInstalled(options = {}) {
  ...
  try {
    ...
    const binary = resolveMagickBinary(options);
    if (binary === 'magick' || binary === 'convert') {
      return false;
    }
    try {
      run(binary, ['-version'], { stdio: 'ignore' });
      return true;
    } catch {
      return false;
    }
...
const HOMEBREW_MANUAL_INSTALL_COMMAND =
...
/**
 * Bekannte magick-Pfade auf macOS (Issue #145-Folge): GUI-gestartete Prozesse
 * erben kein Homebrew-PATH - `magick -version` schlaegt faelschlich fehl,
 * obwohl `brew install imagemagick` erfolgreich war.
 */
const MAGICK_BIN_CANDIDATES_DARWIN = ['/opt/homebrew/bin/magick', '/usr/local/bin/magick'];
 ...
 *
 * `options.force` (Issue #142): "ImageMagick neu installieren/reparieren"
 * im Wartungsmenue (#138) versprach eine echte Reparatur, `brew install`
 * tut bei einer bereits vorhandenen Formula aber nichts. `brew reinstall`
 * erzwingt die Neuinstallation.
 *
 * @param {object} [options]
 * @param {Function} [options.execFileSync] austauschbar fuer Tests
 * @param {boolean} [options.force] erzwingt "brew reinstall" statt "brew install"
...
function installImageMagickMacOS(options = {}) {
  ...
  if (!brewBinary) {
    try {
      ...
      if (/Need sudo access|Administrator/i.test(stderr)) {
        return {
          installed: false,
          error:
            'ImageMagick bleibt optional: Das eingebaute macOS-Tool sips ist aktiv. ' +
            'Homebrew konnte ohne Administratorrechte nicht installiert werden. ' +
            `Terminalbefehl für eine manuelle Homebrew-Installation: ${HOMEBREW_MANUAL_INSTALL_COMMAND}`,
        };
      }
      return {
        installed: false,
  ...
  const verb = options.force ? 'reinstall' : 'install';
  try {
    run(brewBinary, [verb, 'imagemagick'], { stdio: ['ignore', 'ignore', 'pipe'] });
    return { installed: true, error: null };
  } catch (err) {
    const stderr = err.stderr ? err.stderr.toString().trim() : '';
    return {
      installed: false,
      error: `"brew ${verb} imagemagick" fehlgeschlagen: ${stderr || err.message}`,
    };
  }
}
...
function installImageMagickWindows(options = {}) {
  ...
  const run = options.execFileSync || defaultExecFileSync;
  // 7za.exe-Bootstrap: 7-Zip Standalone als .zip laden, Expand-Archive entpackt
  // es nativ; danach 7za.exe zum Entpacken des LZMA-.7z nutzen (Issue #172).
  const psCommand = [
    '$ErrorActionPreference = "Stop";',
    '$ProgressPreference = "SilentlyContinue";',
    `$destDir = ${psQuote(destDir)};`,
    'New-Item -ItemType Directory -Force -Path $destDir | Out-Null;',
    '$szTmp = Join-Path $env:TEMP "kurspilot-7za";',
    'New-Item -ItemType Directory -Force -Path $szTmp | Out-Null;',
    "Write-Host 'Hilfswerkzeug wird geladen...';",
    `Invoke-WebRequest -Uri ${psQuote(WINDOWS_7ZA_BOOTSTRAP_URL)} -OutFile (Join-Path $szTmp '7za.zip');`,
    "Expand-Archive -Force -Path (Join-Path $szTmp '7za.zip') -DestinationPath $szTmp;",
    "Write-Host 'ImageMagick wird heruntergeladen...';",
    '$archivePath = Join-Path $destDir "imagemagick.7z";',
    `Invoke-WebRequest -Uri ${psQuote(WINDOWS_PORTABLE_ZIP_URL)} -OutFile $archivePath;`,
    "Write-Host 'ImageMagick wird entpackt...';",
    '$szExe = Join-Path $szTmp "7za.exe";',
    '& $szExe x $archivePath "-o$destDir" -y;',
    'if ($LASTEXITCODE -ne 0) { throw "7za fehlgeschlagen (Exit $LASTEXITCODE)" };',
    "Write-Host 'ImageMagick fertig entpackt.';",
    'Remove-Item $archivePath;',
    'Remove-Item $szTmp -Recurse -Force;',
  ].join(' ');
 ...
 * @param {boolean} [options.force] erzwingt Neuinstallation/Reparatur, auch wenn
 *   `magick -version` bereits erfolgreich laeuft (Issue #142, "neu
 *   installieren/reparieren" aus #138)
...
function installImageMagick(options = {}) {
  ...
  if (!options.force && isImageMagickInstalled(options)) {
    return { installed: true, error: null };
  }
...
module.exports = {
  ...
  HOMEBREW_MANUAL_INSTALL_COMMAND,
  MAGICK_BIN_CANDIDATES_DARWIN,
};
