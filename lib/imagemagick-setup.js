 ...
 * nichts).
 *
 * `installImageMagickWindows` ist Windows-only und von lib/setup-flow.js als
 * Default-DI ohne Argumente genutzt - seit Issue #133 portabler Zip-Download
 * statt winget (siehe Kommentar bei WINDOWS_PORTABLE_ZIP_URL unten, UAC-Grund).
 * `installImageMagick` ist die arch-erkennende, OS-uebergreifende Erweiterung
 * (Datentabelle wie lib/node-provision.js, lib/app-provision.js): ein neues
 * Ziel ist ein zusaetzlicher Tabelleneintrag, kein Umbau.
 */
...
/**
 * Portable Q16-x64-Zip-Distribution von imagemagick.org (Issue #133).
 *
 ...
 *
 * Loesung: wie win32-arm64 (siehe IMAGEMAGICK_DOWNLOAD_TABLE) direkt die
 * portable, selbstenthaltene Zip-Distribution von imagemagick.org laden und
 ...
 * PowerShell-Download via Invoke-WebRequest + Expand-Archive in
...
function installImageMagickWindows(options = {}) {
  ...
  const psCommand = [
    ...
    `$zipPath = Join-Path ${psQuote(destDir)} 'imagemagick.zip';`,
    `Invoke-WebRequest -Uri ${psQuote(WINDOWS_PORTABLE_ZIP_URL)} -OutFile $zipPath;`,
    `Expand-Archive -Path $zipPath -DestinationPath ${psQuote(destDir)} -Force;`,
    'Remove-Item $zipPath;',
  ].join(' ');

  try {
    run('powershell.exe', ['-NoProfile', '-Command', psCommand], { stdio: 'ignore' });
