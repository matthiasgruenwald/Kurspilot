 ...
 * ImageMagick/releases) als .zip und .7z vor. Wir nutzen die .zip-Variante,
 * damit `Expand-Archive` (PowerShell-Bordmittel) ohne externe Abhaengigkeit
 * entpacken kann - Windows bsdtar (System32\tar.exe) unterstuetzt kein LZMA
 * und scheitert daher an .7z (Issue #172).
...
const WINDOWS_PORTABLE_ZIP_URL =
  ...
  'ImageMagick-7.1.2-26-portable-Q16-x64.zip';
...
function installImageMagickWindows(options = {}) {
  ...
  const psCommand = [
    ...
    `$archivePath = Join-Path ${psQuote(destDir)} 'imagemagick.zip';`,
    ...
    `Expand-Archive -Force -Path $archivePath -DestinationPath ${psQuote(destDir)};`,
