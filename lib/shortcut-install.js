...
function buildShortcutVbScript({ nodePath, setupScriptPath, appPath }) {
  const lines = [
    'Set oWS = WScript.CreateObject("WScript.Shell")',
    `sLinkFile = oWS.SpecialFolders("Programs") & "\\${SHORTCUT_NAME}.lnk"`,
...
function installWindowsShortcut({ homeDir, nodePath, appPath, writeFile, execFile }) {
  ...
  const vbsContent = buildShortcutVbScript({ nodePath, setupScriptPath, appPath });
  writeFile(vbsPath, vbsContent);
  const actualShortcutPath = String(execFile('cscript.exe', ['//nologo', vbsPath]) || '').trim();

  return { platform: 'win32', shortcutPath: actualShortcutPath || shortcutPath };
}
...
function buildMacLauncherScript({ nodePath, setupScriptPath, logPath }) {
  ...
  return [
    ...
    // Die .app selbst darf nicht bis zum Ende des Konfigurationsdienstes
    // laufen. macOS wuerde einen weiteren Klick sonst nur aktivieren statt
    // den Launcher erneut auszufuehren.
    `nohup ${shellQuote(nodePath)} ${shellQuote(setupScriptPath)} >> ${shellQuote(logPath)} 2>&1 &`,
    '',
  ].join('\n');
}
