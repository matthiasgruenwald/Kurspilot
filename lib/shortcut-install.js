 ...
 *     Admin-Rechte i.d.R. nicht beschreibbar).
...
function buildMacLauncherScript({ nodePath, setupScriptPath }) {
  return [
    '#!/bin/sh',
    `exec ${shellQuote(nodePath)} ${shellQuote(setupScriptPath)}`,
    '',
  ].join('\n');
}
...
function installMacShortcut({ homeDir, nodePath, appPath, writeFile, mkdirSync }) {
  ...
  writeFile(launcherPath, buildMacLauncherScript({ nodePath, setupScriptPath }));
  try {
