 ...
 * Fachprofils: `<schuljahr>/<klasse>/<fach>/materials/<topic>`.
 *
 ...
 *   <schuljahr>/<klasse>/<fach>/materials/<topic>/
 ...
 * @param {string} [context.contextRoot] expliziter Arbeitsbereich; sonst wird die Arbeitsbereich-Einstellung gelesen
 ...
 */
function saveMaterial(filePath, context, options = {}) {
  const ctx = context || {};
  ...
  const resolvedContextRoot = resolveKurspilotContextRoot({
    contextRoot,
  ...
  const absMaterialsDir = path.join(resolvedContextRoot, relMaterialsDir);
  const absOriginalDir = path.join(absMaterialsDir, ORIGINAL_DIR);
  assertInsideLocalContext(absMaterialsDir, resolvedContextRoot);
  ...
  const absJournalPath = path.join(resolvedContextRoot, relJournalPath);
  ...
  appendJournalEntry(absJournalPath, entry, { contextRoot: resolvedContextRoot });
