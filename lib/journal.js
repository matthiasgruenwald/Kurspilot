 ...
 *   "2025-26/7a/journal-2026-06-11.md"
 */
 ...
 * @param {string} [options.contextRoot] konfigurierter Kurspilot-Arbeitsbereich fuer Schreibschutz
 */
function appendJournalEntry(filePath, entryMarkdown, options = {}) {
  if (options.contextRoot) {
    assertInsideLocalContext(filePath, options.contextRoot);
  }
...
function recordWorkflowNote(contextRoot, input, options) {
  ...
  appendJournalEntry(absJournalPath, entryMarkdown, { contextRoot: root });
 ...
 * Zeigt erst den Lehrkraftnamen, die Moodle-ID nur als technische Referenz.
 */
function formatCreatedItem(item) {
  const labelPart = item.activityLabel ? `${item.activityLabel}: ` : '';
  const idPart = item.cmid !== undefined ? ` (Moodle-ID ${item.cmid})` : '';
  const linkPart = item.link ? ` - ${item.link}` : '';
  return `- ${labelPart}${item.name}${idPart}${linkPart}`;
}
 ...
 * @param {Array<{name: string, activityLabel?: string, cmid?: number|string, link?: string}>} planResult.created
