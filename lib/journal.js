 ...
 *
 * @param {string} contextRoot Basisverzeichnis, in dem `local-context/` liegt
 * @param {object} input
 ...
 */
function recordWorkflowNote(contextRoot, input) {
  const root = requireNonEmptyText(contextRoot, 'contextRoot');
  const ctx = input || {};
  const note = ctx.note || {};
