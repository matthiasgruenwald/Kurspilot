...
const { normalizeMcQuestionInput, answersToFormParams } = require('./mc-question');
...
const QUESTION_BANK_TOOLS = [
  ...
  },
  {
    ...
    description: "Legt eine Multiple-Choice-Frage an. Bestehende options[] + correctindex-Aufrufe bleiben Einfachauswahl. Für neue Fragen answers[] mit answer, correct, feedback und fraction sowie selectionmode single oder multiple verwenden.",
    inputSchema: {
      type: "object",
      properties: {
        ...
        selectionmode:   { type: "string", enum: ["single", "multiple"], description: "Einfach- oder Mehrfachauswahl" },
        answers:         { type: "array", items: { type: "object", properties: { answer: { type: "string" }, correct: { type: "boolean" }, feedback: { type: "string" }, fraction: { type: "number" } }, required: ["answer"] }, description: "Strukturierte Antworten mit Gewicht zwischen -1 und 1" },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      required: ["questionid", "name", "questiontext"],
    },
    async handler(args, callMoodle) {
      const normalized = normalizeMcQuestionInput(args);
  ...
  {
    name: "moodle_upload_question_image",
    description: "Lädt eine lokal gespeicherte Bilddatei in den Fragetext (questiontext) oder ein Antwortfeedback (answerfeedback) einer MC-Frage hoch und liefert ein @@PLUGINFILE@@-Snippet. Das Snippet danach per moodle_update_mc_question in questiontext bzw. answers[].feedback einfügen (zweiphasiges Muster). Ein zweiter Upload mit demselben Dateinamen überschreibt die vorhandene Datei.",
    inputSchema: {
      type: "object",
      properties: {
        questionid: { type: "number", description: "questionid der Frage (latest version), aus moodle_get_question/moodle_create_mc_question/moodle_update_mc_question" },
        area:       { type: "string", enum: ["questiontext", "answerfeedback"], description: "Zielbereich" },
        answerid:   { type: "number", description: "question_answers.id, Pflicht bei area=answerfeedback (aus moodle_get_question/moodle_create_mc_question answerids)" },
        ...
        alt:        { type: "string", description: "Alternativtext für das Bild", default: "" },
      },
      required: ["questionid", "area", "filepath"],
    },
    async handler(args, callMoodle) {
      ...
      if (args.area === "answerfeedback" && !args.answerid) {
        throw new Error("moodle_upload_question_image: answerid ist bei area=answerfeedback erforderlich.");
      }
      const fileBuffer = fs.readFileSync(filepath);
      const base64     = fileBuffer.toString('base64');
      const filename    = args.filename || path.basename(filepath);

      return await callMoodle("local_coursepilot_upload_question_image", {
        questionid: args.questionid,
        area:       args.area,
        answerid:   args.answerid ?? 0,
        filename:   filename,
        content:    base64,
        alt:        args.alt || "",
      });
  ...
  {
    name: "moodle_move_question",
    description: "Verschiebt eine Frage nicht-destruktiv in eine Zielkategorie, auch zwischen Fragensammlungen. Alle Versionen, Dateien und Schlagwörter bleiben über Moodles Core-Verschiebepfad erhalten.",
    inputSchema: {
      type: "object",
      properties: {
        questionid:       { type: "number", description: "questionid einer beliebigen Version der Frage (aus moodle_get_question)" },
        targetcategoryid: { type: "number", description: "ID der Ziel-Fragenbank-Kategorie" },
      },
      required: ["questionid", "targetcategoryid"],
    },
    async handler(args, callMoodle) {
      return await callMoodle("local_coursepilot_move_question", {
        questionid: args.questionid,
  ...
  },
  {
    ...
    description: "Importiert Moodle-XML-Fragen (z.B. STACK oder andere ueber create_mc_question nicht abbildbare Fragetypen) versionstreu in eine Fragenbank-Kategorie. Wiedererkennung ueber idnumber (Namens-Fallback nur ohne idnumber im XML): ein eindeutiger Treffer erzeugt eine neue Version desselben Fragenbank-Eintrags (Quiz-Slots mit 'immer aktuellste Version' folgen automatisch), ein Verdachtsfall (idnumber ohne Treffer oder Namenstreffer ohne idnumber) schreibt nichts und muss mit allownew=true erneut bestaetigt werden. Ungueltiges XML bricht den gesamten Aufruf ohne Teilergebnis ab.",
    inputSchema: {
      type: "object",
      properties: {
        ...
        allownew:   { type: "boolean", description: "Bestaetigt einen zuvor gemeldeten Verdachtsfall und legt ihn als neuen Eintrag an", default: false },
      },
      required: ["categoryid", "xmlcontent"],
    },
    async handler(args, callMoodle) {
      return await callMoodle("local_coursepilot_import_questions_xml", {
        categoryid: args.categoryid,
        xmlcontent: args.xmlcontent,
        allownew:   (args.allownew ?? false) ? 1 : 0,
      });
    },
  },
  {
    name: "moodle_plan_question_category_cleanup",
    description: "Plant eine manuelle Bereinigung leerer, blattloser Testkategorien in der ausgewaehlten benannten Kurs-/Projekt-Fragensammlung (Kategorien ohne eigene Fragen und ohne Unterkategorien). Kurspilot loescht nichts automatisch: die Antwort nennt jede betroffene Kategorie und den direkten Moodle-Link zur manuellen Pruefung/Loeschung. Solche entarteten Testkategorien koennen einen Moodle-Core-Restore-Fehler ausloesen (#315).",
    inputSchema: {
      type: "object",
      properties: {
        courseid:       { type: "number", description: "Kurs-ID" },
    ...
    readOnly: true,
    async handler(args, callMoodle) {
      return await callMoodle("local_coursepilot_get_question_category_cleanup_plan", {
        courseid:       args.courseid,
        questionbankid: args.questionbankid,
      });
    },
