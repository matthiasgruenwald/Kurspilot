...
const {
  installKurspilotSkillsForProvider: defaultInstallSkillsForProvider,
  areKurspilotSkillsAliased: defaultAreKurspilotSkillsAliased,
 ...
 * und ADR 0008). `updated` reicht das Idempotenz-Ergebnis von provisionApp
 * durch: false heisst, der Tarball-Inhalt war bereits aktuell und es wurde
 * nichts entpackt - dann ist auch kein Dienstneustart noetig.
 *
 * @param {object} [options]
 ...
 * @param {string[]} [options.clients] welche Anbieter installiert werden sollen (Default: beide)
   * @param {string} [options.homeDir]
   * @returns {Promise<{installed: boolean, updated: boolean, offline: boolean, error: string|null, skillInstallAborted: boolean, skillInstallWarnings: string[], skillInstallConflicts: string[], skillInstallConflictPrompts: {skillName: string, prompt: string}[]}>}
   */
async function applyAppUpdate(options = {}) {
  const {
    ...
    areKurspilotSkillsAliased: areKurspilotSkillsAliasedFn = defaultAreKurspilotSkillsAliased,
  ...
  const canonicalSkillRoot = CLIENTS.codex.skillTargetRoot(homeDir);
  const claudeSkillRoot = CLIENTS.claude.skillTargetRoot(homeDir);
  const claudeUsesSharedStorage = areKurspilotSkillsAliasedFn(canonicalSkillRoot, claudeSkillRoot);
  const installedTargetRoots = new Set();
  ...
  for (const client of clients) {
    ...
    if (installedTargetRoots.has(targetRoot) || (client === 'claude' && claudeUsesSharedStorage)) {
      continue;
    }
    installedTargetRoots.add(targetRoot);
  ...
  return {
    installed: true,
    updated: Boolean(provisionResult.updated),
    offline: false,
    error: null,
    appDir,
