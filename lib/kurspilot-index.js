...
/**
 * Rendert ein eigenstaendiges, auf ein Vorhaben begrenztes Katalogblatt
 * (siehe docs/specs/0010, Abschnitt "Thematischer Index": "Ein
 * Materialpaket enthaelt ein auf das Vorhaben begrenzte Katalogblatt.").
 * Nutzt dieselben Felder wie ein Indexeintrag, aber ohne die
 * Marker-Kommentare von `updateIndexEntry` - das Katalogblatt ist eine
 * eigene Datei, kein Eintrag zum Zusammenfuehren mit einem index.md.
 *
 * @param {{data: object, body: string}} vorhabenContext Ergebnis von readFrontmatterFile(CONTEXT.md)
 * @returns {string}
 */
function renderKatalogblatt(vorhabenContext) {
  const { data, body } = vorhabenContext;
  const tagsLine = Array.isArray(data.tags) && data.tags.length ? data.tags.join(', ') : NOT_YET_PROVIDED;
  const lines = [
    '# Katalogblatt',
    '',
    `## ${data.title}`,
    `- Fach: ${data.about}`,
    `- Jahrgangsstufe: ${data.gradeLevel}`,
    `- Tags: ${tagsLine}`,
    `- Kurzbeschreibung: ${extractKurzbeschreibung(body)}`,
    `- Status: ${data.status}`,
  ];
  if (data.license) {
    lines.push(`- Lizenz: ${data.license}`);
  }
  lines.push('');
  return lines.join('\n');
}
