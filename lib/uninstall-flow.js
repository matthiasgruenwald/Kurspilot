...
function runUninstallFlow(options = {}) {
  ...
  const openCodeSkillsResult = removeSkillsForProviderFn(path.join(homeDir, '.agents', 'skills'));
  if (openCodeSkillsResult.removed.length > 0) {
    skillsRemoved.push('opencode');
  }
