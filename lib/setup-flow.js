...
function defaultEndClaudeDesktop(options = {}) {
  ...
  try {
    if (platform === 'win32') {
      // execFileSync leitet stderr ohne explizites stdio an den Elternprozess
      // (die sichtbare Server-Konsole) weiter - taskkill meldet dort sonst
      // "FEHLER: ..." fuer jeden nicht killbaren Kindprozess, obwohl der Catch
      // hier den Fehlschlag bereits abfaengt.
      run('taskkill', ['/IM', 'claude.exe', '/F', '/T'], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] });
      return true;
    }
    if (platform === 'darwin') {
  ...
  for (const target of targets) {
    try {
      run(platform === 'win32' ? 'taskkill' : 'killall', platform === 'win32' ? ['/IM', target, '/F', '/T'] : [target], {
        encoding: 'utf8',
        // s.o. defaultEndClaudeDesktop: stderr sonst ungefiltert in der Server-Konsole.
        ...(platform === 'win32' ? { stdio: ['ignore', 'pipe', 'pipe'] } : {}),
      });
      attempted = true;
    } catch {
...
/**
 * Mindestkonfiguration (Issue #202, Parent #200/Spec 0005): die fuenf
 * Bedingungen, die erfuellt sein muessen, damit die Wartungs-Ansicht statt der
 * Ersteinrichtungs-Ansicht gezeigt wird. Bewusst eine einzelne, wiederverwendete
 * Bedingungsliste - isMinimumConfigured() ist "alle erfuellt",
 * computeSetupProgress() zaehlt genau diese Liste, damit beide synchron bleiben.
 *
 * ponytail: Schritt 0 ("Kurspilot-Programm laeuft") ist tautologisch erfuellt,
 * solange dieser Code ausgefuehrt wird (#229). Dadurch laeuft der Zaehler von
 * 1/6 bis 6/6 statt 0/5 bis 5/5; die Invariante bleibt erhalten.
 */
function minimumConditions(status) {
  return [
    true, // ponytail: tautologischer Schritt 0 (#229)
    Boolean(status.moodle.url),
    Boolean(status.moodle.tokenPresent),
    Boolean(status.workspace.configured),
    Object.values(status.detectedClients).some(Boolean),
    !status.kurspilotRepairRequired,
  ];
}

function isMinimumConfigured(status) {
  return minimumConditions(status).every(Boolean);
}

function computeSetupProgress(status) {
  const conditions = minimumConditions(status);
  return {
    done: conditions.filter(Boolean).length,
    total: conditions.length,
  };
}

/**
 * Naechste offene Bedingung fuer das Fortschrittsband der Wartungsansicht
 * (Issue #230, Spec 0007): bildet den Index der ersten unerfuellten Bedingung
 * aus minimumConditions() auf die zustaendige Card und eine Nutzerbeschriftung
 * ab. Bewusst an minimumConditions() gekoppelt, damit Zaehlung und Fuehrung
 * nicht auseinanderlaufen. Schritt 0 ist tautologisch (#229), daher liegt der
 * erste offene Index immer bei 1-5; null bedeutet "alles erfuellt".
 */
const NEXT_OPEN_CONDITION_BY_INDEX = {
  1: { cardId: 'moodle', label: 'Moodle-URL' },
  2: { cardId: 'moodle', label: 'Moodle-Token' },
  3: { cardId: 'workspace', label: 'Arbeitsordner' },
  4: { cardId: 'clients', label: 'KI-Clients' },
  5: { cardId: 'clients', label: 'KI-Clients einrichten' },
};

function nextOpenCondition(status) {
  const index = minimumConditions(status).findIndex(open => !open);
  return NEXT_OPEN_CONDITION_BY_INDEX[index] || null;
}
