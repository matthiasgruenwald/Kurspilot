 ...
 *
 * ponytail: prueft nur die App "Codex", nicht welche Sitzung -
...
function defaultIsCodexRunning(options = {}) {
  ...
  try {
    ...
    if (platform === 'darwin') {
      const output = run('osascript', ['-e', 'application "Codex" is running'], { encoding: 'utf8' });
      return output.trim() === 'true';
    }
    return false;
  } catch {
...
function defaultEndCodex(options = {}) {
  ...
  try {
    if (platform === 'win32') {
      run('taskkill', ['/IM', 'codex.exe', '/F', '/T'], { encoding: 'utf8' });
      return true;
    }
    if (platform === 'darwin') {
