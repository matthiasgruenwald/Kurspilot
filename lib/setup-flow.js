...
function defaultGetClientSetupStatus(detectedClients, options = {}) {
  ...
  const claudeConfigPath = getClaudeDesktopConfigPath(homeDir);
...
// Windows client detection override for installer builds. 
defaultDetectClients = function defaultDetectClients(options) { 
  options = options ? options : {}; 
  const homeDir = options.homeDir ? options.homeDir : os.homedir(); 
  const platform = options.platform ? options.platform : process.platform; 
  const pathEnv = Object.hasOwn(options, 'pathEnv') ? options.pathEnv : process.env.PATH; 
  if (platform !== 'win32') { 
    return { 
      codex: commandExistsOnPath('codex', pathEnv) || executableExists(path.join(homeDir, '.local', 'bin', 'codex')), 
      claude: fs.existsSync('/Applications/Claude.app') || commandExistsOnPath('claude', pathEnv), 
    }; 
  } 
  const appData = options.appData ? options.appData : (process.env.APPDATA ? process.env.APPDATA : path.join(homeDir, 'AppData', 'Roaming')); 
  const localAppData = options.localAppData ? options.localAppData : (process.env.LOCALAPPDATA ? process.env.LOCALAPPDATA : path.join(homeDir, 'AppData', 'Local')); 
  return { 
    codex: commandExistsOnPath('codex', pathEnv) || fs.existsSync(path.join(homeDir, '.codex')) || fs.existsSync(path.join(appData, 'Codex')) || fs.existsSync(path.join(localAppData, 'Codex')) || fs.existsSync(path.join(localAppData, 'OpenAI', 'Codex')), 
    claude: commandExistsOnPath('claude', pathEnv) || fs.existsSync(path.join(appData, 'Claude')) || fs.existsSync(path.join(localAppData, 'Claude')) || fs.existsSync(path.join(localAppData, 'Programs', 'Claude')) || fs.existsSync(path.join(localAppData, 'AnthropicClaude')), 
  }; 
}; 
module.exports.defaultDetectClients = defaultDetectClients;
 
// Windows installer client-path overrides. 
defaultDetectClients = function defaultDetectClients(options) { 
  options = options ? options : {}; 
  const homeDir = options.homeDir ? options.homeDir : os.homedir(); 
  const platform = options.platform ? options.platform : process.platform; 
  ...
  if (platform !== 'win32') { 
    return { 
      codex: commandExistsOnPath('codex', pathEnv) || executableExists(path.join(homeDir, '.local', 'bin', 'codex')), 
      claude: fs.existsSync('/Applications/Claude.app') || commandExistsOnPath('claude', pathEnv) || fs.existsSync(path.join(homeDir, '.claude')), 
    }; 
  } 
  const appData = options.appData ? options.appData : (process.env.APPDATA ? process.env.APPDATA : path.join(homeDir, 'AppData', 'Roaming')); 
  const localAppData = options.localAppData ? options.localAppData : (process.env.LOCALAPPDATA ? process.env.LOCALAPPDATA : path.join(homeDir, 'AppData', 'Local')); 
  return { 
    codex: commandExistsOnPath('codex', pathEnv) || fs.existsSync(path.join(homeDir, '.codex')) || fs.existsSync(path.join(appData, 'Codex')) || fs.existsSync(path.join(localAppData, 'Codex')) || fs.existsSync(path.join(localAppData, 'OpenAI', 'Codex')), 
    claude: commandExistsOnPath('claude', pathEnv) || fs.existsSync(path.join(homeDir, '.claude')) || fs.existsSync(path.join(appData, 'Claude')) || fs.existsSync(path.join(localAppData, 'Claude')) || fs.existsSync(path.join(localAppData, 'Programs', 'Claude')) || fs.existsSync(path.join(localAppData, 'AnthropicClaude')), 
  }; 
}; 
const originalRunSetupFlowForWindowsPaths = module.exports.runSetupFlow; 
runSetupFlow = function runSetupFlow(options) { 
  options = options ? options : {}; 
  if (process.platform !== 'win32' || options.setupClaudeDesktopConfig) return originalRunSetupFlowForWindowsPaths(options); 
  const homeDir = options.homeDir ? options.homeDir : os.homedir(); 
  const appData = process.env.APPDATA ? process.env.APPDATA : path.join(homeDir, 'AppData', 'Roaming'); 
  const windowsOptions = { ...options, setupClaudeDesktopConfig: function(_configPath, startMcpPath, nodeExecPath, setupOptions) { return setupClaudeDesktopConfig(path.join(appData, 'Claude', 'claude_desktop_config.json'), startMcpPath, nodeExecPath, setupOptions); } }; 
  return originalRunSetupFlowForWindowsPaths(windowsOptions); 
}; 
module.exports.defaultDetectClients = defaultDetectClients; 
module.exports.runSetupFlow = runSetupFlow;
