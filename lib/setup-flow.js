...
const { setupClaudeDesktopConfig, setupClaudeCodeConfig, setupCodexConfig, getSelectedActivityIds } = require('./mcp-config-setup');
...
function defaultIsClaudeDesktopRunning(options = {}) {
  ...
  try {
    ...
    if (platform === 'darwin') {
      // Issue #96-Folgefehler (Live-Test-Befund): "pgrep -x Claude" verfehlte
      // das echte Claude.app, weil aktuelle macOS-Versionen den comm-Namen
      // entitlement-bedingt als vollen Bundle-Pfad statt als blosses "Claude"
      // melden - der exakte Treffer schlug deshalb fehl, obwohl die App lief.
      // "-f" gegen den Bundle-Pfad-Suffix ist robuster und matcht nicht
      // versehentlich eine offene Claude-Code-CLI-Sitzung.
      run('pgrep', ['-f', 'Claude.app/Contents/MacOS/Claude'], { encoding: 'utf8' });
      return true;
    }
    return false;
  } catch {
...
/**
 * Erkennt plattformabhaengig, ob ein Codex-CLI-Prozess laeuft (Issue
 * #96-Folgefehler, Pendant zu defaultIsClaudeDesktopRunning).
 *
 * ponytail: prueft nur den Prozessnamen "codex", nicht welche Sitzung -
...
function defaultIsCodexRunning(options = {}) {
  ...
  try {
    if (platform === 'win32') {
      const output = run('tasklist', ['/FI', 'IMAGENAME eq codex.exe'], { encoding: 'utf8' });
      return /codex\.exe/i.test(output);
    }
    if (platform === 'darwin') {
      run('pgrep', ['-x', 'codex'], { encoding: 'utf8' });
      return true;
    }
    return false;
  } catch {
...
/**
 * Beendet alle laufenden Codex-CLI-Prozesse (Issue #96-Folgefehler, Pendant
 * zu defaultEndClaudeDesktop).
 *
 * ponytail: killall/taskkill auf den Prozessnamen, exakt wie bei Claude -
 * triffe damit auch unbeteiligte, parallel offene Codex-Sitzungen. Bewusst
 * akzeptiertes Risiko (gleiche Holzhammer-Logik wie bei Claude), keine
 * Sitzungs-/PID-Auswahl gebaut.
 */
function defaultEndCodex(options = {}) {
  ...
  try {
    if (platform === 'win32') {
      run('taskkill', ['/IM', 'codex.exe', '/F'], { encoding: 'utf8' });
      return true;
    }
    if (platform === 'darwin') {
      run('killall', ['codex'], { encoding: 'utf8' });
      return true;
    }
    return false;
  } catch {
