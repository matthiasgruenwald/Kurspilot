...
const { setupClaudeDesktopConfig, setupClaudeCodeConfig, setupCodexConfig, getSelectedActivityIds, readConfiguredActivityIds: defaultReadConfiguredActivityIds } = require('./mcp-config-setup');
 ...
 *
 * Seit der ChatGPT-Codex-Zusammenlegung wird auf die ChatGPT-App geprüft.
 ...
 * Windows nutzt mangels Live-Verifikation in dieser Session die analoge
 * Annahme "ChatGPT.exe"; die Konstante hält das notfalls an einer Stelle.
 *
...
function defaultIsCodexRunning(options = {}) {
  ...
  try {
    if (platform === 'win32') {
      const output = run('tasklist', ['/FI', `IMAGENAME eq ${CODEX_WINDOWS_EXECUTABLE}`], { encoding: 'utf8' });
      return new RegExp(CODEX_WINDOWS_EXECUTABLE.replace('.', '\\.'), 'i').test(output);
    }
    if (platform === 'darwin') {
      const output = run('osascript', ['-e', `application "${CODEX_DESKTOP_APP_NAME}" is running`], { encoding: 'utf8' });
      return output.trim() === 'true';
    }
    return false;
  } catch {
...
function defaultEndCodex(options = {}) {
  ...
  try {
    if (platform === 'win32') {
      run('taskkill', ['/IM', CODEX_WINDOWS_EXECUTABLE, '/F', '/T'], { encoding: 'utf8' });
      return true;
    }
    if (platform === 'darwin') {
      run('killall', [CODEX_DESKTOP_APP_NAME], { encoding: 'utf8' });
      return true;
    }
    return false;
  } catch {
...
function buildSetupStatus(options = {}) {
  ...
  const configuredActivityIds = readConfiguredActivityIdsFn({
    ...
    claudeDesktopConfigPath: getClaudeDesktopConfigPath(homeDir),
  });
...
function runSetupFlow(options = {}) {
  ...
  const codexSkillTargetRoot = path.join(homeDir, '.codex', 'skills');
