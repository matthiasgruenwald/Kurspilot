...
const MAINTENANCE_AREAS = [
  ...
  {
    id: 'crop-backend-prefer-imagemagick',
    label: 'ImageMagick statt sips als Standard für den Bildausschnitt verwenden',
  },
  {
    id: 'no-change',
    label: 'Nichts ändern',
...
function buildMaintenanceSelection(status) {
  ...
  const sipsActive = Boolean(status.imageMagick && status.imageMagick.sipsActive);
  const bothBackendsAvailable = sipsActive && imageMagickAvailable;

  let areas = imageMagickSupported
  ...
  if (bothBackendsAvailable) {
    if (status.imageMagick.preferredBackend === 'imagemagick') {
      preselectedAreaIds.push('crop-backend-prefer-imagemagick');
    }
  } else {
    areas = areas.filter(area => area.id !== 'crop-backend-prefer-imagemagick');
  }
...
function runSetupFlow(options = {}) {
  ...
  // Issue #139: der Backend-Schalter ist nur eine echte Wahl, wenn beide
  // Crop-Tools auf diesem System verfuegbar sind - sonst gibt es nichts zu
  // entscheiden, und ein Schreiben wuerde nur eine nicht angebotene Option
  // (siehe buildMaintenanceSelection) vortaeuschen.
  const bothCropBackendsAvailable = platform === 'darwin' && isSipsAvailableFn() && isImageMagickAvailableFn();
  if (usesMaintenanceSelection && bothCropBackendsAvailable) {
    const preferImageMagick = selectedMaintenanceAreas.includes('crop-backend-prefer-imagemagick');
    writeCropBackendPreferenceFn(preferImageMagick ? 'imagemagick' : 'sips', { homeDir });
    executedSteps.push(
      preferImageMagick
